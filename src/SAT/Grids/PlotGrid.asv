...
% field Ant.Obj().Name. Pass an empty matrix (any string) for PObjs, 
...
elseif iscell(PObjs),
  for n=1:length(PObjs),
    PObjs{n}=GridFindObj(Ant,'Name',PObjs{n});
  end
  PObjs=cat(1,PObjs{:});
end
PObjs=PObjs(:);

[PPoints,PWires,PSurfs]=GridFindObj(Ant);
...
for n=PPoints(:)',
  if ~isempty(Ant.Obj(n).GraphProp),
    set(cat(1,NH{abs(Ant.Obj(n).Elem)}),Ant.Obj(n).GraphProp);
  end
end
...
for n=PWires(:)',
  if ~isempty(Ant.Obj(n).GraphProp),
    set(cat(1,SH{abs(Ant.Obj(n).Elem)}),Ant.Obj(n).GraphProp);
  end
end
...
for n=PSurfs(:)',
  if ~isempty(Ant.Obj(n).GraphProp),
    set(cat(1,PH{abs(Ant.Obj(n).Elem)}),Ant.Obj(n).GraphProp);
  end
end
