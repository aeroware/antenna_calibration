...
if (nargin<4)|isempty(P),  % determine default P
  P=-1;
end
if isequal(P,-1),
...
if (length(x)==3)&(length(n)==1),  % 2-dimensional plot
  ...
  G=feval(GainFunction,Ant,Op,r,feed,P);
...
%  [hp,hc,ha,hco,c]=PlotGain(Ant,Op,P,x,y,n,LogScale,ColScale,GContour,tmax)
  ...
  G=reshape(feval(GainFunction,Ant,Op,u(:)*x+v(:)*y+w(:)*z,feed,P),size(t));
  ...
  G=reshape(feval(GainFunction,Ant,Op,[u(:),v(:),w(:)],feed,P),size(t));
