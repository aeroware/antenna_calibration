...
Scale=1/200*100;            % cm assumed, m given

LineWidth=0.3*Scale;        % wire line width
aLinewidth=3*Scale;       % antenna line width
fLinewidth=0.6*Scale;       % feed segment linewidth
aLinecolor=[1,0,0]*0.8;
fLinecolor=[1,0.5,0];
aLineStyle='-';

EdgeWidth=0.15*Scale;        % line width of patch edges
EdgeColor=[1,1,1]*0.2;

hAStyle={'Color','red','LineWidth',aLinewidth,'LineStyle',aLineStyle};
...
xlim([-11 11]);  xlabel('+X Axis [m]');
ylim([-3 3]);  ylabel('+Y Axis [m]');
zlim([-12 8]);   zlabel('+Z Axis [m]');
% 
