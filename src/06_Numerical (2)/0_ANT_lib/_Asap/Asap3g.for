C     Last change:  F    20 Aug 2003   11:17 am
C     Inserted line 1429 INTERGER*4 I and removed I from preceding line;
C     Inserted line 797 DATA BLANK/' '/ (which was originally included 
C     in the previous line in the character definition), similarly at 1682; WM. 2009.
C     Changed lines for LOAD handling (previously at ~2320), Macher 2001.
C     Added double precision value for constants as pi, 2pi, 4pi, eps0, 
C     180/pi, pi/180, Euler etc., replaced all REAL by DREAL, all AIMAG
C     by DIMAG, all ABS by DABS, all CSQRT by CDSQRT, all CABS by CDABS,
C     all CEXP by CDEXP, all ALOG by DLOG, Fischer 2003.
C     Subroutine EXPJ for calculation of exponential integrals with complex
C     arguments was replaced by a new routine, Fischer 2003.	
      PROGRAM ASAP
C     DOUBLE PRECISION   
C
...
C
C       ASAP VERSION 3.1D DOUBLE PRECISION VERSION FOR PC/MAINFRAME/WORKSTATION
C
...
C
      ...
      DATA E0,U0/8.854187817E-12,1.2566370614E-6/
C
...
C
   ...
   34 PH = PHAI-STEP
     ...
     3AM,ERR,IGRD)
      ETMAG = CDABS(ETTS)
      EPMAG = CDABS(EPPS)
      IF(ETMAG.GT.1.E-32) PHSTH=57.29577951308232*ATAN2(DIMAG(ETTS),
     4DREAL(ETTS))
      IF(EPMAG.GT.1.E-32) PHSPH=57.29577951308232*ATAN2(DIMAG(EPPS),
     5DREAL(EPPS))
...
C         POWER INPUT CHANGED TO E11.5 FORMAT
  ...
  740 FORMAT (' ',T50,37(' ')/T50,'*',T86,'*'/
     ...
     6 T50,'*                                   *'/
     7 T50,'*  VERSION 3.1D DOUBLE PRECISION    *'/
     8 T50,'*           (MARCH 1998)            *'/
     9 T50,'*',T86,'*'/T50,37('*'))
   ...
   96 FORMAT (T50,'WIRE STRUCTURE'//T4,'SEG',5X,2('NODE',19X,'LOCATION'
     1,23X)/T5,'NO.',4X,2(' NO.',9X,'X',14X,'Y',14X,'Z',10X)/(T3,I4,5X,
     22(I4,5X,E11.5,4X,E11.5,4X,E11.5,2X)))
C    I2 changed to I4 for ASAP output file (segment and node numbers)
...
C
    ...
    5 B01 = (.0,-1.)
      ...
      DATA PI/3.141592653589793/
      GD = GAM*DK
      CST = (EP2-EP)*ETA*DLOG(BM/AM)/(4.*PI*EP2*SGDS*SGDS)
...
C
    ...
    2 RETURN
      ...
      REAL*8 X1, Y1, X2, Y2, TH, AB 
      COMPLEX*16 V1, V2, Z, CE1, CE2, W12
C New subroutine for calculation of exponential integral with complex
C arguments. Replaced Aug. 2003, G. Fischer.      
      CALL E1Z(V1,CE1)
      CALL E1Z(V2,CE2)
      Z = V2/V1
     ...
     1(V1),REAL(V1))
      AB = ABS(TH)
      IF (AB.LT.1.) TH = .0
      IF (TH.GT.1.) TH = 6.283185307179586
      IF (TH.LT.-1.) TH = -6.283185307179586
      W12 = CE1-CE2+CMPLX(.0,TH)
      END
...
C    1 DISS = DISS+FA*(CDABS(CJA)**2+CDABS(CJB)**2)+FB*(DREAL(CJA)*
C     1DREAL(CJB)+DIMAG(CJA)*DIMAG(CJB))
C
    1 DISS = DISS+DABS(FA*(CDABS(CJA)**2+CDABS(CJB)**2))+DABS(FB*(DREAL
     1(CJA)*DREAL(CJB)+DIMAG(CJA)*DIMAG(CJB)))
C
C
    2 DO 3 J=1,NM
      K = J+NM
    3 DISS = DISS+DREAL(ZLD(J))*(CDABS(CG(J))**2)+DREAL(ZLD(K))*(CDABS
     1(CG(K))**2)
C
      RETURN
      END
    ...
    3 CONTINUE
     ...
     3,ETA,GAM,ERR,IGRD)
      ...
      CJI = -4.*PI/(ETA*GAM)
      GGG = DREAL(1./ETA)
      THR = 0.01745329251994330*TH
      CTH = COS(THR)
      STH = SIN(THR)
      PHR = 0.01745329251994330*PH
      CPH = COS(PHR)
      SPH = SIN(PHR)
C
      DO 1 I=1,N
...
C
      ...
      VP = CJI*EPP(I)
      VT = CJI*ETT(I)
      PIN = PIN+DREAL(VP*CONJG(EP(I)))
    5 TIN = TIN+DREAL(VT*CONJG(ET(I)))
C
      ECSP = PIN/GGG
      ECST = TIN/GGG
      SCSP = ECSP-ACSP
...
C
    8 AMS = AM*AM
     ...
     1L2,TD1,TD2,SDI,DPSI,DD,ZD
      ...
      BD = CD*DPSI
      B = BD
      EB = CDEXP(GAM*CMPLX(.0,B))
      EC = CDEXP(GAM*CMPLX(.0,C))
C
      DO 1 K=1,2
C
      DO 1 L=1,2
...
C
      DO 1 IA=1,N
      D = IA-1
    1 X(IA) = D*3.141592653589793/180.0
C
C
...
C     SET UP EQUATIONS FOR MULTIPLES OF 30 DEGREES
    6 X = 61.0+1.732050807568877*Y*S
      CALL LINECK (X,Y)
      X = 61.0+Y*S/1.732050807568877
    7 CALL LINECK (X,Y)
C
...
C
   ...
   18 IF ((A(1).NE.AI).OR.(A(2).NE.AMA).OR.(A(3).NE.AP).OR.(A(4).NE.AE))
     1 GO TO 22
      KFLAG(24) = 1
   19 I = 1
C      IF(KFLAG(24).EQ.1) I=LOAD+1  commented out,
C      KFLAG(24) = 1		moved 3 lines up, Macher 2001.	
      CALL LEFT (N)
...
C
      ...
      PA = .0
      PB = .0
      IF (ACJ.GT.0.) PA = 57.29577951308232*ATAN2(DIMAG(CJA),DREAL(CJA))
      IF (BCJ.GT.0.) PB = 57.29577951308232*ATAN2(DIMAG(CJB),DREAL(CJB))
...
C
    6 FORMAT (1H0)
    7 FORMAT (2X,I4,2(2X,I4,2X,E21.15,1X,E21.15,1X,E11.5,1X,E11.5,1X,
     1F6.1))
    8 FORMAT (/2(71X,'NORMALIZED',1X)/'   SEG',2('   NODE',9X,'REAL',
     116X,'IMAGINARY',8X,'MAGNITUDE',3X,'MAGNITUDE',3X,'PHASE'))
      END
...
C
     ...
     1M,BM,C,CGD,CMM,D,EP2,EP3,ETA,FHZ,GAM,SGD,X,Y,Z,ZLD,ZS,ERR,IGRD)
      ...
      DATA E0,TP,U0/8.854187817E-12,6.283185307179586,1.2566370614E-6/
      DATA ZERO/0.0000/
      DATA ONE/1.0000/
      EP = EP3
      ICC = (N*N+N)/2
...
C
      ...
      IF (CDABS(GAM*DMAX).GT.3.) GO TO 4
      IF (AM.GT.0.) GO TO 5
    4 CONTINUE
C     N=0
...
C
      ...
      SNOR = SA/CNOR
      PH = .0
      IF (SA.GT.0.) PH = 57.29577951308232*ATAN2(DIMAG(SS),DREAL(SS))
