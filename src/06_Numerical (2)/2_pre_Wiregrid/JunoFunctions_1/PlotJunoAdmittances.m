%   PlotJunoAdmittances
%
%   This script makes three plots of the Admittances of the Juno model.
%   It plots the real part, the imaginary part and the real vs. imaginary 
%   part.
%
%   Written by Thomas Oswald, September 2007
%   ref. code. 837647326728-15
...
for(n=1:length(f))
    ...
    Adms(n,:,:)=reshape(AntAdmittance(ant, OP,solver,0,25.4/1000),[1,2,2]);
...
title(strcat('Admittances: Real and Imaginary Part vs. Frequency',strcat('/',titl)));
lab=cellstr(['                                         '
             'Admittance-Real Part/Ohm                 '
             'Admittance-Imaginary Part/Ohm            '
             '                                         '
             'Admittances: Real Part vs. Imaginary Part']);
...
legend('E_1','E_2')
title(strcat('Admittances: Real Part vs. Imaginary Part',strcat('/',titl)));
