...
1     WRITE(*,'('' Enter INPUT file name (or RETURN) >'',$)')
	...
	IF(INFILE.EQ.' ')THEN
     ...
     &   QWIN$MSG_FILEOPENDLG)
        OPEN (UNIT=2,FILE="",STATUS='OLD',READONLY,ERR=3)
	ELSE
...
2     WRITE(*,'('' Enter OUTPUT file name (or RETURN) >'',$)')
     ...
     &   QWIN$MSG_FILEOPENDLG)
         OPEN (UNIT=3,FILE=" ",STATUS='UNKNOWN',ERR=4)
	ELSE
...
C=======================================================================
C
C     FBARS evaluates FBARS(Z)=1.-SQRT(PI)*Z*EXP(Z*Z)*ERFC(Z)
...
C
     ...
     &CFF2,CFF3,CFF4,CFF5,CFF6,CFF7,CFF8,ZZEXP
      COMMON /GPARM/ XLA(21,21),GCK1,GCK1SQ,GCK2,GCK2SQ,ZRAT,RHON
      COMMON /FPARM/ CFF1,CFF2,CFF3,CFF4,CFF5,CFF6,CFF7,CFF8,FF1,FF2,
     &FF3,FF4,FF5,FF6,FF7,FF8,FF9,FF10,FF11,FF12,FF13,FF14,FF15,FF16,
     &FF17,FF18,FF19,FF20,FF21,FF22,FF23,FF24,NFUNF,NFUNC
...
C     BB=TAN(ABS(TOTALLY REFELECTING ANGLE)+ATAN(BLIM))
