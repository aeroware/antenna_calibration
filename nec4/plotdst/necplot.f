...
C     FILE.
C
C     THIS PROGRAM MUST BE LINKED WITH THE DIGLIB GRAPHICS LIBRARY ON A
C     DEC/VAX COMPUTER.  DIGLIB IS AVAILABLE FROM LAWRENCE LIVERMORE LAB
C
      CHARACTER*60 INFILE
      CHARACTER*2 AIN
      COMMON/PLTPRMN/XPLEN,YPLEN,SCALZ,VTHET,VPHI,VETA,XCEN,YCEN,ZXFAC,
     &ICSEG,ICURNT,IENDSY,IAROW,CRMAX,CIMAX,CRIMAX,FRQMHZ,INFILE,INFIO,
     &IDOTEN,SCALEF,SCAROW,XMN,YMN,NPLOT
C
...
C
      INCLUDE 'NECPAR.INC'
      CHARACTER INFILE*60,AIN*2,MESGE*48
      BYTE COM
C      INTEGER*2 GPWNXY(2)
C      LOGICAL*4 LGTPT,GETPUT
     ...
     &IDOTEN,SCALEF,SCAROW,XMN,YMN,NPLOT
      ...
      MESGE='Enter name of nec input or output file          '
      NPAT=0
      NPLOT=NPLOT+1
      FRQMHZ=0.
...
703   WRITE(*,700)
700   FORMAT(' ENTER NAME OF NEC INPUT OR OUTPUT FILE >',$)
...
C703   GPWNXY(1)=50
C      GPWNXY(2)=100
C      LGTPT= GetPut(1,GPWNXY,MESGE,INFILE,IVOL,1,'TEXT')
C      IF(.NOT.LGTPT)STOP
...
125   FORMAT(A2,80A1)
      IF (AIN.EQ.'CM'.OR.AIN.EQ.'cm') GO TO 2
      IF (AIN.EQ.'CE'.OR.AIN.EQ.'ce') THEN
           CALL DATAGN
           INFIO=0
      ELSE IF(AIN.EQ.'1 ')THEN
           CALL REDOUT
           INFIO=1
      ELSE
           WRITE(*,90)
...
90         FORMAT(' ERROR - END OF FILE ENCOUNTERED READING NEC INPUT FI
     &LE')
      ...
      SUBROUTINE REDOUT
      INCLUDE 'NECPAR.INC'
      CHARACTER INFILE*60,TEXT*80,TEXT1*4
      BYTE COM
...
4     IF(N.EQ.0)GO TO 1
      ...
      ICURNT=1
      DO 10 I=1,N
10    READ(2,96)CR(I),CI(I)
96    FORMAT(48X,2E12.5)
...
C            WRITE(*,62) SEGL1,XS1,YS1,ZS1
         ELSE IF(IX.EQ.2)THEN
            NS=0
            CALL RDLSOL(NS,XW1,YW1,ZW1,XW2,YW2,ZW2,SEGL1,SEGL2,XS1)
 C           WRITE(*,67) SEGL1,SEGL2,YS1,ZS1,NS,XS1
         END IF
