...
C
      ...
      COMPLEX  CM,ZARRAY,CEINS,ZPEDA,AX,BX,CX,ZPED,AIX,BIX,CIX,CUR,XKS,
     &XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2,TLYT1,TLYT2,YN11,
...
C***
     ...
     &ICLIFT,CLIFL,CLIFH,EPSR2,SIG2,NRADL,SCNRAD,SCNWRD,KSYMP,IFAR,
     &IPERF,GSCAL
     ...
     &INTAGF(MAXIS),INTAGT(MAXIS),EPSIN(MAXIS),SIGIN(MAXIS),RADIN(MAXIS)
      COMMON/ZPSAV/ZPEDA(200),NZSAVE,NZLIMT,ISZSAV,FRMHZA(200),ZPNORM
      COMMON /SEGJ/ AX(NSJMAX),BX(NSJMAX),CX(NSJMAX),JCO(NSJMAX),JSNO,
     &ISCON(NSCNGF),NSCON,IPCON(NSPNGF),NPCON
      COMMON /SORCES/ NSORC,ISORTP(NSOMAX),PSOR1(NSOMAX),PSOR2(NSOMAX),
     &PSOR3(NSOMAX),PSOR4(NSOMAX),PSOR5(NSOMAX),PSOR6(NSOMAX),DTHINC,
     &DPHINC,NTHINC,NPHINC,NSCINC
     ...
     &NVSORA(NSOMAX),ISEG1(NETMX),ISEG2(NETMX),NONET,NETYP(NETMX)
      COMMON/NETCX/ZPED,IZSAVE,PIN,PNLS,NPEQ,NTSOL,NPRINT
...
C
      ...
      COMPLEX AIX,BIX,CIX,CUR,XKS,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,
     &ETAL2
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     ...
     &IPERF,GSCAL
      ...
      WRITE(LUNPLT)N
      DO 1 I=1,N
1     WRITE(LUNPLT)AIX(I),BIX(I),CIX(I),SI(I),XKS(I)
      RETURN
      END
      SUBROUTINE GEOOUT
C=======================================================================
...
C
     ...
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
      ...
      WRITE(LUNPLT)N
      DO 1 I=1,N
1     WRITE(LUNPLT)I,X(I),Y(I),Z(I),SI(I),ALP(I),BET(I),SALP(I),
     &ICON1(I),ICON2(I),ITAG(I)
      RETURN
      END
...
C=======================================================================
      ...
      IF(PLFNAM.EQ.' '.OR.FILNAM.NE.' ')THEN
         ...
         OPEN(UNIT=LUNPLT,FILE=PLFNAM,FORM='UNFORMATTED',STATUS=
     &   'unknown')
      END IF
      RETURN
      END
...
C
      COMPLEX XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,ETAL2,FRATI,XLA,GCK1,
...
C
      ...
      CEPSU=CMPLX(UEPS,-USIG/(TP*FMHZ*1.E6*EPSRZ))
      CEPSL=CEPSU
      XK=SQRT(CEPSU)
...
C
      ...
      ELSE
         ...
         EPSC1=CMPLX(GEPS,-GSIG/(TP*FMHZ*1.E6*EPSRZ))
         CEPSL=EPSC1
         XK=SQRT(EPSC1)
      ...
      IF(ICLIFT.NE.0)THEN
         ...
         EPSC2=CMPLX(EPSR2,-SIG2/(TP*FMHZ*1.E6*EPSRZ))
         XK=SQRT(EPSC2)
         ETAL2=ETAZ/XK
      END IF
      IF(IPERF.NE.1)THEN
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX TLYT1,TLYT2,YN11,YN12,YN22,SVOLTS,XKU,XKL,ETAU,ETAL,CEPSU,
     &CEPSL,FRATI,ETAL2
      COMMON/NETDEF/TLYT1(NETMX),TLYT2(NETMX),YN11(NETMX),YN12(NETMX),
     &YN22(NETMX),SVOLTS(NSOMAX),TLZCH(NETMX),TLLEN(NETMX),NVSOR,
     &NVSORA(NSOMAX),ISEG1(NETMX),ISEG2(NETMX),NONET,NETYP(NETMX)
...
C
      INCLUDE 'NECPAR.INC'
      CHARACTER PLFNAM*60
      COMPLEX ZARRAY,TLYT1,TLYT2,YN11,YN12,YN22,ENINTG,HNINTG,SVOLTS,
...
C
C*** VAX:
C      IF (INFILE .NE. ' ')OPEN (UNIT=2,FILE=INFILE,STATUS='OLD',
C     &                    READONLY,ERR=3)
C
C*** MACINTOSH:
C      IF (INFILE .NE. ' ')OPEN (UNIT=2,FILE=INFILE,STATUS='OLD',
...
C
C*** UNIX:
      IF (INFILE .NE. ' ')OPEN (UNIT=2,FILE=INFILE,STATUS='OLD',ERR=3)
C
      IERR=0
...
C
C*** VAX:
C      IF (OUTFILE .NE. ' ')OPEN (UNIT=3,FILE=OUTFILE,STATUS='NEW',ERR=4)
C
C*** UNIX:
      IF (OUTFILE .NE. ' ')OPEN (UNIT=3,FILE=OUTFILE,STATUS='UNKNOWN',
     &ERR=4)
      WRITE(*,94)INFILE
      WRITE(*,96)OUTFILE
      WRITE(*,95)
...
C
      COMPLEX XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2
      COMMON /RPDAT/ THETS,DTH,NTH,PHIS,DPH,NPH,IPD,IAVP,INOR,IAX,RFLD,
     &GNOR
      COMMON /GND/XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,ETAL2,FRATI,OMEGAG,
     &ICLIFT,CLIFL,CLIFH,EPSR2,SIG2,NRADL,SCNRAD,SCNWRD,KSYMP,IFAR,
     &IPERF,GSCAL
...
C
      INCLUDE 'NECPAR.INC'
      CHARACTER*2 AIN
      COMPLEX TLYT1,TLYT2,YN11,YN12,YN22,SVOLTS
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
     ...
     &NVSORA(NSOMAX),ISEG1(NETMX),ISEG2(NETMX),NONET,NETYP(NETMX)
      ...
      IF (AIN.EQ.'NT') THEN
           NETYP(NONET)=1
           YN11(NONET)=CMPLX(TMP1,TMP2)
           YN12(NONET)=CMPLX(TMP3,TMP4)
           YN22(NONET)=CMPLX(TMP5,TMP6)
           RETURN
      ELSE
...
C
     ...
     &          (Z(ISG2)-Z(ISG1))**2)
           END IF
           TLLEN(NONET)=TMP2
           TLYT1(NONET)=CMPLX(TMP3,TMP4)
           TLYT2(NONET)=CMPLX(TMP5,TMP6)
      END IF
      RETURN
C
...
C
1     CALL SORVT1(IROUND(PSOR1(ISOR)),CMPLX(PSOR2(ISOR),PSOR3(ISOR)),E)
      GO TO 10
C
...
C
4     CALL SORVQD(IROUND(PSOR1(ISOR)),CMPLX(PSOR2(ISOR),PSOR3(ISOR)),E)
10    CONTINUE
      RETURN
      END
      SUBROUTINE SORINI(E)
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX E,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2,FJ,CX,CY,
     ...
     &ICLIFT,CLIFL,CLIFH,EPSR2,SIG2,NRADL,SCNRAD,SCNWRD,KSYMP,IFAR,
     &IPERF,GSCAL
      DIMENSION CAB(1),SAB(1),T1X(1),T1Y(1),T1Z(1)
      DIMENSION E(*)
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX E,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2,XKSEG,FJ,
     ...
     &XO,YO,ZO,ISNOR,IREG,IDIRX
      DIMENSION E(*),CAB(1),SAB(1),T1X(1),T1Y(1),T1Z(1),EGND(9)
      EQUIVALENCE (CAB,ALP),(SAB,BET),(T1X,SI),(T1Y,ALP),(T1Z,BET)
      COMMON/CONSTN/PI,TP,DTORAD,CVEL,EPSRZ,RMUZ,ETAZ
      DATA FJ/(0.,1.)/
      DDZ=COS(DALP)
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX VOLTS,E,AX,BX,CX,XKSJ,EXK,EYK,EZK,EXS,EYS,EZS,EXC,EYC,EZC,
     &ZPEDS,ZARRAY,CEINS,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2,
     ...
     &XKS(MAXSEG)
      ...
      DIMENSION E(*),CAB(1),SAB(1),T1X(1),T1Y(1),T1Z(1)
      EQUIVALENCE (CAB,ALP),(SAB,BET),(T1X,SI),(T1Y,ALP),(T1Z,BET)
      COMMON/CONSTN/PI,TP,DTORAD,CVEL,EPSRZ,RMUZ,ETAZ
      DATA CCJ/(0.,-.01666666667)/
      I=ICON1(ISEG)
      ICON1(ISEG)=0
      CALL TBF (ISEG,0)
      ICON1(ISEG)=I
      SLENJ=SI(ISEG)*.5
      XKSJ=XKS(ISEG)
      CURD=CCJ*VOLTS/((ALOG(2.*SLENJ/BI(ISEG))-1.)*(BX(JSNO)*COS(XKSJ*
...
19    IF (NLOAD.GT.0.OR.NLODF.GT.0) E(J)=E(J)+ZARRAY(J)*CURD*AX(JX)*XKU
      IF(NINS.NE.0.OR.NINSF.NE.0)THEN
         IF(BRINS(J).GT.0.)THEN
            ...
            CFCON=CFCON*(0.,1.)*ALOG(BRINS(J)/BI(J))/(TP*CEINS(J))
            E(J)=E(J)+CFCON*CX(JX)
         END IF
      END IF
20    CONTINUE
...
C
      COMPLEX YSTE,ZSTM,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2,
...
C
      ...
      IF(ABS(REAL(CTHM)).LT.ABS(AIMAG(CTHM))*1.E-6)THEN
         IF(CTH*AIMAG(CTHM).GE.0.)CTHM=-CTHM
      END IF
      YSTE=CTHM/ETAX
      ZSTM=CTHM*ETAX
...
C
      ...
      ZSCRN=(0.,1.)*RMUZ*OMEGAG*RHOSCN/NRADL*ALOG(RHOSCN/(SCNWRD*NRADL))
...
C
      COMPLEX ETA,YSTE,ZSTM,EX,EY,EZ,RX,RY,RZ,YZTE,ZZTM,REFTE,REFTM,ENOR
      IF(IPERFG.EQ.1)THEN
C
...
C
      ...
      IF(REAL(CKZX).EQ.0..AND.CTH*AIMAG(CKZX).LT.0.) CKZX=-CKZX
      RETURN
      END
...
C                INTERFACE
C     
      COMPLEX CKZ,CKZX,XK,XKX,EX,EY,EZ,TX,TY,TZ,TZZ,TRR,TPP,ERHO,EPHI
      IF(AIMAG(XK).NE.0.)THEN
C
...
C     
     ...
     &ICLIFT,CLIFL,CLIFH,EPSR2,SIG2,NRADL,SCNRAD,SCNWRD,KSYMP,IFAR,
     &IPERF,GSCAL
      CALL REFPT(0.,DX,DY,DZ,XP,YP,ZP,XS,YS,ZS)
...
C
      INCLUDE 'NECPAR.INC'
      PARAMETER (IRESRV=MAXMAT**2)
      COMPLEX EINC,CM,TLYT1,TLYT2,YN11,YN12,YN22,SVOLTS,CMN,RHNX,RHNT,
     &VLTNET,ZPED,VLT,CUX,YMIT
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
...
C
      IF(NSORC.GT.0)THEN
         ...
         IF(ISORTP(I).EQ.1)THEN
            ...
            SVOLTS(NVSOR)=CMPLX(PSOR2(I),PSOR3(I))
         END IF
1        CONTINUE
      END IF
C
...
C
      INCLUDE 'NECPAR.INC'
      PARAMETER (IRESRV=MAXMAT**2)
      COMPLEX EINC,TLYT1,TLYT2,YN11,YN12,YN22,SVOLTS,CMN,RHNX,RHNT,CM,
     &RHS,CURX,VLTNET
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX TLYT1,TLYT2,YN11,YN12,YN22,SVOLTS,CMN,RHNX,RHNT,VLTNET
      COMMON/NETDEF/TLYT1(NETMX),TLYT2(NETMX),YN11(NETMX),YN12(NETMX),
     &YN22(NETMX),SVOLTS(NSOMAX),TLZCH(NETMX),TLLEN(NETMX),NVSOR,
     &NVSORA(NSOMAX),ISEG1(NETMX),ISEG2(NETMX),NONET,NETYP(NETMX)
      COMMON/NETWRK/CMN(NETMX,NETMX),RHNX(NETMX),RHNT(NETMX),
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX VOLTS,CRNT,YMIT,AIX,BIX,CIX,CUR,XKS
      COMMON/CRNT/AIX(MAXSEG),BIX(MAXSEG),CIX(MAXSEG),CUR(3*MAXSEG),
     &XKS(MAXSEG)
      CRNT=CUR(ISEG)
      YMIT=CRNT/VOLTS
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX VOLTS,CRNT,YMIT,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,
...
C
      ...
      ZPEDM=ABS(ZPEDA(I))
      ZPEDP=CANG(ZPEDA(I))
      ZPRNOR=REAL(ZPEDA(I))/ZPNORM
      ZPXNOR=AIMAG(ZPEDA(I))/ZPNORM
      ZPMNOR=ZPEDM/ZPNORM
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX AIX,BIX,CIX,CUR,XKS,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,
     &ETAL2,CURX,CURY,CURZ,CURT1,CURT2,XK
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
...
C
      CHARACTER*6 POLTYP
      COMMON/RECPAT/NPATRC,MAXRCX,ISEGRP,THETRC(200),PHIRC(200),ETARC,
     &ARATRC,PATRC(200),POLTYP
...
C     
      INCLUDE 'NECPAR.INC'
      COMPLEX QSEG,AIX,BIX,CIX,CUR,XKS,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,
     &FRATI,ETAL2,XK,XKSS
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     ...
     &IPERF,GSCAL
      ...
      XKSS=XKS(ISEG)
      QSEG=(0.,1.)*REAL(XK*XK)*XKSS/(OMEGAG*XK**2)*(BIX(ISEG)*
...
C
      CALL WCHARG(0.,ISEG,QSEG,XSNOR,YSNOR,ZSNOR,SINOR)
      QMAG=ABS(QSEG)
      PH=CANG(QSEG)
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX AIX,BIX,CIX,CUR,XKS,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,
...
C
      IF(NLOAD.NE.0.OR.NLODF.NE.0)THEN
           DO 1 I=1,N
           ZARR=REAL(ZARRAY(I)*XKU)
           IF (ABS(ZARR).LT.1.E-20) GO TO 1
           PWLOSS=PWLOSS+REAL(CUR(I)*CONJG(CUR(I)))*ZARR*SI(I)
1          CONTINUE
           PWLOSS=.5*PWLOSS
      END IF
C
...
C        EPCON=-1./(4.*PI*OMEGA*EPSRZ)
         ...
         XK=XKS(I)
         BMAGS=REAL(BIX(I))**2+AIMAG(BIX(I))**2
         XMAGS=REAL(XK)**2+AIMAG(XK)**2
         ARG=REAL(XK)*SI(I)
         ...
         ELSE
            ...
            CMAGS=REAL(CIX(I))**2+AIMAG(CIX(I))**2
            CINT=.5*SI(I)*XMAGS*((SF1+SF2)*BMAGS+(SF2-SF1)*CMAGS)
         END IF
         XMAGS=ALOG(BRINS(I)/BI(I))/(REAL(CEINS(I))**2+AIMAG(CEINS(I)
     &         )**2)
         PSLOSS=PSLOSS+EPCON*AIMAG(CEINS(I))*XMAGS*CINT
3        CONTINUE
      END IF
      RETURN
      END
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX ETH,EPH,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2,
     ...
     &ICLIFT,CLIFL,CLIFH,EPSR2,SIG2,NRADL,SCNRAD,SCNWRD,KSYMP,IFAR,
     &IPERF,GSCAL
      DIMENSION CAB(1),SAB(1),S(1)
      EQUIVALENCE (CAB,ALP),(SAB,BET),(S,BI)
      DATA CONS/(0.,.07957747151)/
      CONST=-XKU*ETAU*CONS
      SPH=SIN(PHI)
      CPH=COS(PHI)
      STH=SIN(THET)
...
C
      ...
      IF (ABS(AIMAG(XK)).LT.ABS(REAL(XK))*1.E-6) GO TO 3
      ETH=(0.,0.)
      EPH=(0.,0.)
      RETURN
C
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX XK,CKX,CKY,CKZ,EX,EY,EZ,AIX,BIX,CIX,CUR,XKS,FJ,OMEGA,XKK,
     &SILL,TOP,BOT,A,QI,ARG,EXA,CURX,CURY,CURZ
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX XK,ETA,EX,EY,EZ,AIX,BIX,CIX,CUR,XKS,FJ,CKX,CKY,CKZ,OMEGA,
     &XKK,SILL,TOP,BOT,A,QI,ARG,EXA,CEX,CEY,CEZ,YSTE,ZSTM,RX,RY,RZ,CURX,
     &CURY,CURZ
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
...
2     ITAG(I)=ITG
      RETURN
C
3     FORMAT (' ARC: ERROR - ARC ANGLE EXCEEDS 360. DEGREES')
      END
      FUNCTION ATGN2 (X,Y)
C=======================================================================
C     (C) Copyright 1992
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX CM,H1X,H1Y,H1Z,H2X,H2Y,H2Z,G11,G12,G21,G22
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX CM,CW,ETK,ETS,ETC,XKSJ,EXK,EYK,EZK,EXS,EYS,EZS,EXC,EYC,
     ...
     &ICLIFT,CLIFL,CLIFH,EPSR2,SIG2,NRADL,SCNRAD,SCNWRD,KSYMP,IFAR,
     &IPERF,GSCAL
      DIMENSION CM(NR,*), CW(NW,*), CAB(1), SAB(1)
      DIMENSION T1X(1), T1Y(1), T1Z(1)
      EQUIVALENCE (CAB,ALP), (SAB,BET), (T1X,SI), (T1Y,ALP), (T1Z,BET)
      LDP=LD+1
      SLENJ=SI(J)
...
C
      COMPLEX Y11,Y12,Y21,Y22,YY12,CRHO,YLOAD,YIN,ZLOAD,ZIN
C
      WRITE(3,6)
      ...
      YY12=Y12*Y21
      C=ABS(YY12)/(2.*REAL(Y11)*REAL(Y22)-REAL(YY12))
      ...
      ELSE
         GMAX=(1.-SQRT(1.-C*C))/C
      END IF
      CRHO=GMAX*CONJG(YY12)/ABS(YY12)
      YLOAD=((1.-CRHO)/(1.+CRHO)+1.)*REAL(Y22)-Y22
...
C
      INCLUDE 'NECPAR.INC'
      PARAMETER (IRESRV=MAXMAT**2)
      COMPLEX Y11,Y12,Y21,Y22,CM,TLYT1,TLYT2,YN11,YN12,YN22,SVOLTS,CMN,
     &RHNX,RHNT,VLTNET,ZPED,EXCIT,VLT
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
...
C     &ACCESS='DIRECT',RECL=2*LREC(IUNIT))
C***  UNIX:  RECL IN BYTES
      OPEN(UNIT=IUNIT,FILE=FIL,STATUS=STAT,FORM='UNFORMATTED',
     &ACCESS='DIRECT',RECL=8*LREC(IUNIT))
      RETURN
      END
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX VOLTS,E,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2,XK,
...
C
      COMPLEX XK,EX,EY,EZ,ECR1,ECR2,ECZ1,ECZ2
      SHALF=.5*SLENJ
...
C
      COMPLEX Z,ZX,ZS,SUM,POW,TERM
      DATA TOSP/1.128379167/,ACCS/1.E-12/,SP/1.772453851/
      Z=ZX
...
C
      COMPLEX ERV,EZV,ERH,EPH,EZH,RRV,RZV,RRH,RPH,RZH,CK1,CK1SQ,CK2,CK2S
     &Q,CKS12,CON1,CON2,CON3,CON4,EPSC1,XJK,SAV
...
C
      COMPLEX E1,E2,E3,E4,E5,FAC1,FAC2,FAC3,FAC4,FAC5,EXX,REXX,CFF1,CFF2
     &,CFF3,CFF4,CFF5,CFF6,CFF7,CFF8,CFF,FAC1L,FAC2L,FAC3L,FAC4L,FAC5L
...
C
      COMPLEX XLA,GCK1,GCK1SQ,GCK2,GCK2SQ,FJ,XLS,BET1,BET2,EXX,EXM,CFF1,
     &CFF2,CFF3,CFF4,CFF5,CFF6,CFF7,CFF8
      COMMON /GPARM/ XLA(21,21),GCK1,GCK1SQ,GCK2,GCK2SQ,ZRAT,RHON
      COMMON /FPARM/ CFF1,CFF2,CFF3,CFF4,CFF5,CFF6,CFF7,CFF8,FF1,FF2,
     &FF3,FF4,FF5,FF6,FF7,FF8,FF9,FF10,FF11,FF12,FF13,FF14,FF15,FF16,
     &FF17,FF18,FF19,FF20,FF21,FF22,FF23,FF24,NFUNF,NFUNC
...
C
      ...
      XLS=GCK1*RHO/RP
      BET2=-SQRT(GCK2SQ-XLS*XLS)
      EXM=EXP(-FJ*(BET2*ZZ+GCK1*RP))/EXX
      CFF1=EXM*CTH1*STH
      CFF2=CFF1*CTH1
      CFF3=EXM/RP*STH
      CFF4=CFF3*CTH1
...
C
      COMPLEX XLS,EXX,ERV,EZV,ERH,EPH,EZH,XLA,GCK1,GCK1SQ,GCK2,GCK2SQ,
     &CON,FJ,XLSS,CGAM1,CGAM2,RHOXL,ZOG1,ZOG2,PSI,TCV,TCU,ZZEXP
      COMMON /GPARM/ XLA(21,21),GCK1,GCK1SQ,GCK2,GCK2SQ,ZRAT,RHON
      DATA CON/(42.398,-42.398)/,FJ/(0.,1.)/
      XLSS=XLS*XLS
      CGAM1=SQRT(XLSS-GCK1SQ)
      CGAM2=SQRT(XLSS-GCK2SQ)
      IF (REAL(XLS).LE.REAL(GCK2).AND.AIMAG(CGAM2).LT.0.) CGAM2=-CGAM2
...
C
      SAVE
      COMPLEX XLA,GCK1,GCK1SQ,GCK2,GCK2SQ,CK1X,CK2X,CD,AA,XLP,PG1,PG2,
     &FJOR,XLS,XLS2,XLSQ,CGG2,CGG4,CGG,GAM,GAM2,GAM4,DEN,FMHF,DEN32,FT1,
     &FT2,FT3,EXX,T1,T2,T3,T4,T5,T6,T7,DP0,DP1,DP2,GP0,GP1,GP2,U12,ERV,
     &EZV,ERH,EPH,EZH,FS,FP,BB,VP12,FJ,FBARS,GG0,GG2,GX0,GX2,FA2,FB2,
     &FA4,FB4,ZZEXP
      COMMON /GPARM/ XLA(21,21),GCK1,GCK1SQ,GCK2,GCK2SQ,ZRAT,RHON
      COMMON /GASAV/ CD,AA,XLP,PG1,PG2,GG0,GG2,GX0,GX2,FA2,FB2,FA4,FB4
      DATA FJ/(0.,1.)/,CK1X/(0.,0.)/,CK2X/(0.,0.)/
      T1=GCK1-CK1X
      T2=GCK2-CK2X
      ZXX=REAL(GCK2)*1.E-8
      IF (ABS(REAL(T1)).GT.ZXX.OR.ABS(AIMAG(T1)).GT.ZXX) GO TO 1
      IF (ABS(REAL(T2)).GT.ZXX.OR.ABS(AIMAG(T2)).GT.ZXX) GO TO 1
      GO TO 2
...
C
      SAVE
      COMPLEX EZV,ERH,EPH,XLA,GCK1,GCK1SQ,GCK2,GCK2SQ,FJ,CK1X,CK2X,T1,
     &T2,GG0,GG2,GX0,GX2,FA2,FB2,FA4,FB4,FP2,FP4,EXX,U12,CD,AA,XLP,PG1,
     &PG2,ZZEXP
      COMMON /GPARM/ XLA(21,21),GCK1,GCK1SQ,GCK2,GCK2SQ,ZRAT,RHON
      COMMON /GASAV/ CD,AA,XLP,PG1,PG2,GG0,GG2,GX0,GX2,FA2,FB2,FA4,FB4
      DATA FJ/(0.,1.)/,CK1X/(0.,0.)/,CK2X/(0.,0.)/
...
C     of the E field.
      COMPLEX ERV,EZV,ERH,EPH,EZH,XLA,GCK1,GCK1SQ,GCK2,GCK2SQ,XLS,EXX,
     &XLSX,FRV,FZV,FRH,FPH,FZH,VZV,VRH,VPH
      COMPLEX GRV,GZV,GRH,GPH,GZH
      COMMON /GPARM/ XLA(21,21),GCK1,GCK1SQ,GCK2,GCK2SQ,ZRAT,RHON
      DATA ALIM/.2/,BLIM/.1/
      CALL SADPT (RHO,ZZ,ZP,XLS)
      CALL SDLPT (XLS,IERR)
      ...
      BB=ABS(GCK2/GCK1)
      BB=BB/SQRT(1.-BB*BB)
C     BB=TAN(CABS(TOTALLY REFELECTING ANGLE))
...
1     ZXX=(ZZ-ZP)*ALIM
      ...
      IF (ZZ*REAL(GCK2).GT.100.) GO TO 5
C
...
C
      COMPLEX CKFS,CK,ERV,EZV,ERH,EPH,EZH,FJ,CON,EXF,T1,T2
      DATA FJ/(0.,1.)/
      CON=(0.,-29.98)*CKFS
      ZD=ZO-ZS
...
C
      ...
      COMPLEX CM,SSX,ZARRAY,CEINS,EPSC,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,
     &FRATI,ETAL2
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
...
C        READ PATCH DATA
      ...
      IF (IPERF.EQ.2)THEN
         EPSC=CMPLX(GEPS,-GSIG/(TP*FMHZS*1.E6*EPSRZ))
...
C
      ...
      PARAMETER (IRESRV=MAXMAT**2)
      CHARACTER COMIN*78,INFILE*80,OUTFILE*80,NGFFIL*80,FILNAM*(*)
      COMPLEX CM,SSX,ZARRAY,CEINS,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,
     &ETAL2
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
...
C
      COMPLEX ERV,EZV,ERH,EPH,EZH,DRV,DZV,DRH,DPH,DZH,RRV,RZV,RRH,RPH,RZ
     &H,SAV,CK1,CK1SQ,CK2,CK2SQ,CKS12,CON1,CON2,CON3,CON4,EPSC1,XJK,CK
      COMMON /GNRZZ/ CK1,CK1SQ,CK2,CK2SQ,CKS12,CON1,CON2,CON3,CON4,EPSC1
     &,XJK,RHO,ZS,ZO,ZZ,ZZP,AZP,ICASE
      COMMON /GREGON/ RHOA(3),RHOB(3),ZZA(3),ZZB(3),ZPA(3),ZPB(3),ELM,EL
     &MX,SCFAC,RHMX1,ZZMX1,ZPMX1,RHMX2,ZZMX2,ZPMX2,ZPMXX
...
C
      ...
      COMPLEX FAC1,FAC2,FAC3,FAC4,FAC5,A1,A2,A3,A4,A5,CK1,CK1SQ,CK2,CK2S
     &Q,CKS12,CON1,CON2,CON3,CON4,XJK,EPSC1,EPSXX,EPSCX
      COMMON /COFIT/ FAC1(32,3),FAC2(32,3),FAC3(32,3),FAC4(32,3),
     &FAC5(32,3),RKFAC,MFUNF(3),MFUNC(3)
      COMMON /GREGON/ RHOA(3),RHOB(3),ZZA(3),ZZB(3),ZPA(3),ZPB(3),ELM,EL
     &MX,SCFAC,RHMX1,ZZMX1,ZPMX1,RHMX2,ZZMX2,ZPMX2,ZPMXX
      COMMON /GRIDY/ A1(1200),A2(1200),A3(1200),A4(1200),A5(1200),RH1(3)
     &,ZZ1(3),ZP1(3),DRH(3),DZZ(3),DZP(3),NRH(3),NZZ(3),NZP(3),NPT(3),DR
     &ZZP(3),INDI(3),ZPM1,ZPM2,ZPMM1,ZPMM2,ZPD1,ZPD2
      COMMON /GNRZZ/ CK1,CK1SQ,CK2,CK2SQ,CKS12,CON1,CON2,CON3,CON4,EPSC1
     &,XJK,RHO,ZS,ZO,ZZ,ZZP,AZP,ICASE
      COMMON /GNDCOM/ EPSCX,NREG,XRH1,DXRH,NXRH,XZZ1,DXZZ,NXZZ,XZP1,
     &DXZP,NXZP,SRH1,DSRH,NSRH,SZZ1,DSZZ,NSZZ,SZP1,DSZP,NSZP,RES1,RES2,
     &RES3,RES4,RES5,REX1,REX2,REX3,REX4,REX5
      COMMON /GNDFIL/NSFILE,SOMFIL
      DATA PI/3.141592654/
      IF(IFLN.EQ.21)THEN
...
C     unit number IFLN
C
      COMPLEX FAC1,FAC2,FAC3,FAC4,FAC5,A1,A2,A3,A4,A5,EPSCX
      COMMON /COFIT/ FAC1(32,3),FAC2(32,3),FAC3(32,3),FAC4(32,3),
     &FAC5(32,3),RKFAC,MFUNF(3),MFUNC(3)
      COMMON /GREGON/ RHOA(3),RHOB(3),ZZA(3),ZZB(3),ZPA(3),ZPB(3),ELM,EL
     &MX,SCFAC,RHMX1,ZZMX1,ZPMX1,RHMX2,ZZMX2,ZPMX2,ZPMXX
...
C
      COMPLEX H1X,H1Y,H1Z,H2X,H2Y,H2Z,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,
...
C
      COMPLEX XKSJ,EXK,EYK,EZK,EXS,EYS,EZS,EXC,EYC,EZC,ZPEDS,ZRATI,HPK,
     &HPS,HPC,QX,QY,QZ,RRV,RRH,ZRATX,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,
     &FRATI,ETAL2,XK,ETX
      COMMON /DATAJ/ XKSJ,EXK,EYK,EZK,EXS,EYS,EZS,EXC,EYC,EZC,ZPEDS,
     &SLENJ,ARADJ,XJ,YJ,ZJ,DXJ,DYJ,DZJ,IND1,IND2
      COMMON /GND/XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,ETAL2,FRATI,OMEGAG,
...
C
      IF (NRADL.GT.0)THEN
         ...
         IF (RHOSPC.LE.SCNRAD)THEN
            RRV=(0.,1.)*RMUZ*OMEGAG*RHOSPC/NRADL*ALOG(RHOSPC/(NRADL*
     &      SCNWRD))
...
C     transitions.
C
      COMPLEX ERV,EZV,ERH,EPH,EZH,FRV,FZV,FRH,FPH,FZH,A1,A2,A3,A4,A5,CK1
     &,CK2,CK1SQ,CK2SQ,CKS12,CON1,CON2,CON3,CON4,EPSC1,XJK
      COMMON /GRIDY/ A1(1200),A2(1200),A3(1200),A4(1200),A5(1200),RH1(3)
     &,ZZ1(3),ZP1(3),DRH(3),DZZ(3),DZP(3),NRH(3),NZZ(3),NZP(3),NPT(3),DR
     &ZZP(3),INDI(3),ZPM1,ZPM2,ZPMM1,ZPMM2,ZPD1,ZPD2
      COMMON /GNRZZ/ CK1,CK1SQ,CK2,CK2SQ,CKS12,CON1,CON2,CON3,CON4,EPSC1
...
C
      COMPLEX ERV,EZV,ERH,EPH,EZH,RRV,RZV,RRH,RPH,RZH,CK1,CK1SQ,CK2,CK2S
     &Q,CKS12,CON1,CON2,CON3,CON4,EPSC1,XJK,RF1,RF2,RF3,RF4,CONS,XKR1,XK
     &R2,EKR1,EKR2,EXJK,SAV
      COMMON /GNRZZ/ CK1,CK1SQ,CK2,CK2SQ,CKS12,CON1,CON2,CON3,CON4,EPSC1
     &,XJK,RHO,ZS,ZO,ZZ,ZZP,AZP,ICASE
...
C
      COMPLEX E1,E2,E3,E4,E5,A1,A2,A3,A4,A5,CK1,CK1SQ,CK2,CK2SQ,CKS12,CO
     &N1,CON2,CON3,CON4,EPSC1,XJK,EXJK,F1,F2,F3,F4,F5
      COMMON /GRIDY/ A1(1200),A2(1200),A3(1200),A4(1200),A5(1200),RH1(3)
     &,ZZ1(3),ZP1(3),DRH(3),DZZ(3),DZP(3),NRH(3),NZZ(3),NZP(3),NPT(3),DR
     &ZZP(3),INDI(3),ZPM1,ZPM2,ZPMM1,ZPMM2,ZPD1,ZPD2
      COMMON /GNRZZ/ CK1,CK1SQ,CK2,CK2SQ,CKS12,CON1,CON2,CON3,CON4,EPSC1
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX ZARRAY,ZT,ZINT,OMGJ,XK
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
...
C
      ...
      ELSE IF(JUMP.EQ.5)THEN
         ZT=CMPLX(ZLR(ISTEP),ZLI(ISTEP))
      ELSE IF(JUMP.EQ.6)THEN
         RMU=ZLI(ISTEP)
         IF(RMU.EQ.0.)RMU=1.
         ZT=ZINT(ZLR(ISTEP)*WLAM,RMU,BI(I)/WLAM)*SI(I)/WLAM
      END IF
      IF((ABS(REAL(ZARRAY(I)))+ABS(AIMAG(ZARRAY(I)))).GT.1.E-20)IWARN=1
...
C
     ...
     &   ZLI(ISTEP),ZLC(ISTEP),0.,0.,0.,' SERIES ')
      ELSE IF(JUMP.EQ.2)THEN
         CALL PRNT (LDTAGS,LDTAGF(ISTEP),LDTAGT(ISTEP),ZLR(ISTEP),
     &   ZLI(ISTEP),ZLC(ISTEP),0.,0.,0.,'PARALLEL')
      ELSE IF(JUMP.EQ.3)THEN
         CALL PRNT (LDTAGS,LDTAGF(ISTEP),LDTAGT(ISTEP),ZLR(ISTEP),
     &   ZLI(ISTEP),ZLC(ISTEP),0.,0.,0.,' SERIES (PER METER) ')
      ELSE IF(JUMP.EQ.4)THEN
         CALL PRNT (LDTAGS,LDTAGF(ISTEP),LDTAGT(ISTEP),ZLR(ISTEP),
     &   ZLI(ISTEP),ZLC(ISTEP),0.,0.,0.,'PARALLEL (PER METER)')
      ELSE IF(JUMP.EQ.5)THEN
         CALL PRNT (LDTAGS,LDTAGF(ISTEP),LDTAGT(ISTEP),0.,0.,0.,
     &   ZLR(ISTEP),ZLI(ISTEP),0.,'FIXED IMPEDANCE ')
      ELSE IF(JUMP.EQ.6)THEN
         CALL PRNT (LDTAGS,LDTAGF(ISTEP),LDTAGT(ISTEP),0.,0.,0.,0.,0.,
     &   ZLR(ISTEP),'  WIRE  ')
      END IF
      GO TO 2
C
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX ENINTG,HNINTG,EX,EY,EZ
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
...
10    FORMAT (///,38X,'- - - NEAR ELECTRIC FIELDS - - -',//,14X,
     ...
     &'VOLTS/M',3X,'DEGREES')
11    FORMAT (2X,3F12.4,1X,3(3X,1PE11.4,2X,0PF7.2))
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX ENINTG,HNINTG,ENINT,EX,EY,EZ,EA,EH,EV
      COMMON /NFDAT/ ENINTG,HNINTG,XNR,YNR,ZNR,DXNR,DYNR,DZNR,NEAR,NFEH,
     &NRX,NRY,NRZ
      COMMON/CONSTN/PI,TP,DTORAD,CVEL,EPSRZ,RMUZ,ETAZ
      DX=DXNR-XNR
...
94    FORMAT (2X,3F12.4,1X,3(3X,1PE11.4,2X,0PF7.2))
      END
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX HX,HY,HZ,ACX,BCX,CCX,XKSJ,EXK,EYK,EZK,EXS,EYS,EZS,EXC,EYC,
     &EZC,H1X,H1Y,H1Z,H2X,H2Y,H2Z,ZPEDS,AIX,BIX,CIX,CUR,XKS,XKU,XKL,
     &ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2,CON
      COMPLEX EXPX,EXMX,EXPY,EXMY,EXPZ,EXMZ
      COMPLEX EYPX,EYMX,EYPY,EYMY,EYPZ,EYMZ
      COMPLEX EZPX,EZMX,EZPY,EZMY,EZPZ,EZMZ
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
     ...
     &IPERF,GSCAL
      ...
      DIMENSION T1X(1),T1Y(1),T1Z(1),XS(1),YS(1),ZS(1)
...
6     IF (ZOB.LT.0.) GO TO 7
      CON=(0.,1.)/(XKU*ETAU)
      DELT=.00628318/ABS(XKU)
      GO TO 8
7     CON=(0.,1.)/(XKL*ETAL)
      DELT=.00628318/ABS(XKL)
8     IGLOCK=1
...
C
      COMPLEX H1X,H1Y,H1Z,H2X,H2Y,H2Z,E,E1,E2,E3,E4,E5,E6,E7,E8,E9
      COMMON/DATAP/H1X,H1Y,H1Z,H2X,H2Y,H2Z,SPATJ,XPATJ,YPATJ,ZPATJ,T1XJ,
     &T1YJ,T1ZJ,T2XJ,T2YJ,T2ZJ,IPGND
      DIMENSION E(9)
...
C
      ...
      COMPLEX ETH,EPH,ERD,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2
C***
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
...
30    CONTINUE
      IF (IAVP.NE.0)THEN
         TMP3=THETS*DTORAD
         TMP4=TMP3+DTH*DTORAD*FLOAT(NTH-1)
         TMP3=ABS(DPH*DTORAD*FLOAT(NPH-1)*(COS(TMP3)-COS(TMP4)))
         PINT=TP*PINT/TMP3
         AVGAIN=GCOP*PINT/TP
         TMP3=TMP3/PI
...
C     below.
C
      COMPLEX ERV,EZV,ERH,EPH,EZH,CK1,CK1SQ,CK2,CK2SQ,CKS12,CON1,CON2,CO
     &N3,CON4,EPSC1,XJK
      COMMON /GNRZZ/ CK1,CK1SQ,CK2,CK2SQ,CKS12,CON1,CON2,CON3,CON4,EPSC1
     &,XJK,RHO,ZS,ZO,ZZ,ZZP,AZP,ICASE
      RHO=RHOX
...
C
      COMPLEX XLA,GCK1,GCK1SQ,GCK2,GCK2SQ,XZ1,XZ2,XLS
      COMMON /GPARM/ XLA(21,21),GCK1,GCK1SQ,GCK2,GCK2SQ,ZRAT,RHON
      DATA CK1R,CK1I/0.,0./,NDIM/21/,DR,DZ/0.,0./
      IF (REAL(GCK1).NE.CK1R.OR.AIMAG(GCK1).NE.CK1I)THEN
         CK1R=REAL(GCK1)
         CK1I=AIMAG(GCK1)
...
C
      COMPLEX XLA,XL,GCK1,GCK1SQ,GCK2,GCK2SQ,CJRO,COM,CGAM1,CGAM2,TERM
      COMMON /GPARM/ XLA(21,21),GCK1,GCK1SQ,GCK2,GCK2SQ,ZRAT,RHON
      ZMZZ=1.-RHON*RHON
      IF (ZMZZ.LT.0.) ZMZZ=0.
      ZMZZ=SQRT(ZMZZ)
      ...
      IF (AIMAG(COM).LT.0.) CGAM1=-CGAM1
      COM=XL-GCK2
      CGAM2=SQRT(XL+GCK2)*SQRT(COM)
      IF (AIMAG(COM).LT.0.) CGAM2=-CGAM2
      COM=2.*XL*XL
...
2     IERR=2
      GO TO 4
3     IERR=0
      IF (REAL(XL).GT.REAL(GCK2)) IERR=3
4     RETURN
      END
...
C=======================================================================
c
c       stopwtch operates as a stopwatch for CPU time.  When first
c       called, the routine initializes the clock.  On subsequent calls
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX A,B,C,D,SUM,XY,Y,AX,BX,CX
      COMMON /SCRATM/ Y(2*MAXSEG)
      COMMON /SEGJ/ AX(NSJMAX),BX(NSJMAX),CX(NSJMAX),JCO(NSJMAX),JSNO,
     &ISCON(NSCNGF),NSCON,IPCON(NSPNGF),NPCON
      COMMON /MATPAR/ ICASE,NBLOKS,NPBLK,NLAST,NBLSYM,NPSYM,NLSYM,IMAT,I
...
C
      COMPLEX XLA,XL,XLL,GCK1,GCK1SQ,GCK2,GCK2SQ
      COMMON /GPARM/ XLA(21,21),GCK1,GCK1SQ,GCK2,GCK2SQ,ZRAT,RHON
      DZ=1./(NZ-1.)
      DR=1./(NR-1.)
      ZRAT=-DZ
      ...
      IF (IZ.EQ.1) THEN
         XL=GCK1*ROZX
         IF (REAL(XL).GT.REAL(GCK2)) XL=GCK2
         XLA(IR,IZ)=XL
...
C
      COMPLEX ERV,EZV,ERH,EPH,EZH,FRV,FZV,FRH,FPH,FZH,CK1,CK1SQ,CK2,CK2S
     &Q,CKS12,CON1,CON2,CON3,CON4,EPSC1,XJK
      COMMON /GREGON/ RHOA(3),RHOB(3),ZZA(3),ZZB(3),ZPA(3),ZPB(3),ELM,EL
     &MX,SCFAC,RHMX1,ZZMX1,ZPMX1,RHMX2,ZZMX2,ZPMX2,ZPMXX
      COMMON /GNRZZ/ CK1,CK1SQ,CK2,CK2SQ,CKS12,CON1,CON2,CON3,CON4,EPSC1
     &,XJK,RHO,ZS,ZO,ZZ,ZZP,AZP,ICASE
...
C
      ...
      IF(RADIN(IS).LT.BI(I))THEN
         ...
         STOP
      END IF
      CEINS(I)=CMPLX(EPSIN(IS),SIGIN(IS))
...
C
      ...
      XKS(I)=XKI*SQRT(1.-H0/(XK*BRINS(I)*ALOG(BRINS(I)/BI(I))*H0P))
10    CONTINUE
      RETURN
      END
      SUBROUTINE PTSLEN
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,ETAL2,FRATI,AIX,BIX,
     &CIX,CUR,XKS,XK
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
...
92    FORMAT(///)
      END
      COMPLEX FUNCTION ZINT(SIGL,RMU,ROLAM)
C=======================================================================
C     (C) Copyright 1992
C     The Regents of the University of California.  All rights reserved.
C=======================================================================
...
C
     ...
     &(CC12,CCN(23)), (CC13,CCN(25)), (CC14,CCN(27))
      ...
      DATA CMOTP/60.00/,FJ/(0.,1.)/,CN/(.70710678,.70710678)/
      DATA CCN/6.E-7,1.9E-6,-3.4E-6,5.1E-6,-2.52E-5,0.,-9.06E-5,-9.01E-5
     &,0.,-9.765E-4,.0110486,-.0110485,0.,-.3926991,1.6E-6,-3.2E-6,1.17E
     &-5,-2.4E-6,3.46E-5,3.38E-5,5.E-7,2.452E-4,-1.3813E-3,1.3811E-3,-6.
     &25001E-2,-1.E-7,.7071068,.7071068/
C
...
C
         ...
         IF((NINS.NE.0.OR.NINSF.NE.0).AND.BRINS(J).GT.0.)THEN
            ...
            END IF
            ETC=ETC-CFCON*ALOG(BRINS(J)/BI(J))/(TP*CEINS(J))*XKSJ**2/XKU
         END IF
      END IF
C
C     FILL MATRIX ELEMENTS.  ELEMENT LOCATIONS DETERMINED BY CONNECTION
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX AIX,BIX,CIX,CUR,XKS,XKSJ,EXK,EYK,EZK,EXS,EYS,EZS,EXC,EYC,
     &EZC,ZPEDS,H1X,H1Y,H1Z,H2X,H2Y,H2Z,ZARRAY,XKU,XKL,ETAU,ETAL,CEPSU,
     &CEPSL,ETAL2,FRATI,QSEGE1,QSEGE2,EX,EY,EZ,ACX,BCX,CCX
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
...
C
      COMPLEX XKSJ,EXK,EYK,EZK,EXS,EYS,EZS,EXC,EYC,EZC,ZPEDS,XKU,XKL,
...
C
17    DMIN=EXK*CONJG(EXK)+EYK*CONJG(EYK)+EZK*CONJG(EZK)
      DMIN=.01*SQRT(DMIN)
...
C
      ...
      CALL SOMFLD(XI,YI,ZI,XJ,YJ,ZJ,DXJ,DYJ,DZJ,SLENJ,ARADJ,XKSJ,0.,
...
C     EXC,EYC,EZC = X,Y,Z COMPONENTS OF E DUE TO COS(KS)-1 CURRENT
C
      COMPLEX ZPEDS,XK,ETX,XKS,EXK,EYK,EZK,EXS,EYS,EZS,EXC,EYC,EZC,
...
C
      COMPLEX ZRATI,ETX,TXK,TYK,TZK,TXS,TYS,TZS,TXC,TYC,TZC,XKU,XKL,
...
C
      IF (NRADL.NE.0)THEN
         ...
         RHOSPC=SQRT(XSPEC*XSPEC+YSPEC*YSPEC+(NRADL*SCNWRD)**2)
         IF (RHOSPC.LE.SCNRAD)THEN
            ZSCRN=(0.,1.)*RMUZ*OMEGAG*RHOSPC/NRADL*ALOG(RHOSPC/(NRADL*
     &      SCNWRD))
...
C
      COMPLEX ZPEDS,XK,ETA,XKS,EZK,EZS,EZCM,ERK,ERS,ERC,ECR1,ECR2,ECZ1,
     &ECZ2
      COMMON /VLCAPC/IVCAP
      SHALF=.5*SLEN
...
C     EZCM,ERC = Z AND RHO COMPONENTS OF E DUE TO COS(KS)-1 CURRENT
C
      COMPLEX XK,ETA,XKS,EZK,EZS,EZC,ERK,ERS,ERC,RHOEND,EZ,ER,SINKS,
     &COMKS,CCOSM1
      COMMON/CONSTN/PI,TP,DTORAD,CVEL,EPSRZ,RMUZ,ETAZ
      SHALF=.5*SLEN
      IF(IEND.GT.0)THEN
...
C
      COMPLEX XK,ER,EZ,XJKZ,RZK,XKR,XJKR,EXKR,TERM1,TERM2
      ABSZ=ABS(ZDIST)
C
...
C
      COMPLEX XK,ETA,EZK,EZS,EZCM,ERS,ERC
      SHAF=.5*SLEN
      SHS=SHAF*SHAF
      ZDIST=ABS(ZOB)+SHAF
...
C     EXC,EYC,EZC = X,Y,Z COMPONENTS OF E DUE TO COS(KS)-1 CURRENT
C
      COMPLEX XKSX,EXK,EYK,EZK,EXS,EYS,EZS,EXC,EYC,EZC,XKSEG,EGND
      COMMON /INCOM/ XKSEG,SEGL,XSJ,YSJ,ZSJ,DIRX,DIRY,DIRZ,SN,XSN,YSN,
     &XO,YO,ZO,ISNOR,IREG,IDIRX
      DIMENSION EGND(9)
      EXTERNAL SFLDS
...
C
         ...
         IF(RIJS.LT.2.*SLENJ**2)THEN
            CALL ROMBG (-SHAF,SHAF,9,SFLDS,EGND,DMIN,1.E-4)
         ELSE
...
C
      COMPLEX ANS(*),G1(9),G2(9),G3(9)
      DATA WCEN/.4444444/,WOUT/.2777778/
      ABCEN=.5*(A+B)
      BA=B-A
      ABOUT=.3872983*BA
...
C
      COMPLEX E,XKSEG,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2,
...
C
      COMPLEX XK,ETA,EZK,EZS,EZCM,ERS,ERC,CON,CONK,SHK,SS,CS,EXRI,GZ1,
     &GZ2
      DATA CON/(0.,.07957747151)/
      CONK=CON*ETA
      SH=.5*SLEN
      SHK=XK*SH
      SS=SIN(SHK)
...
C
      ...
      GZ1=EXP(-(0.,1.)*XK*R1)/R1
      GZ2=EXP(-(0.,1.)*XK*R2)/R2
      EZS=CONK*(GZ2-GZ1)*CS
      EZCM=-CONK*(GZ2+GZ1)*SS-EZK
      IF(RHO.GT..01*AMIN1(R1,R2).OR.(Z2.GT.0..AND.Z1.LT.0.))THEN
...
C     (CONSTANT CURRENT PRODUCES ZERO RHO COMPONENT OF E)
C
      COMPLEX XK,ETA,XKS,EZK,EZS,EZCM,ERS,ERC,EXI,CON,CONK,XKDIF,SHK,
     &SS,CS,GZ1,GZ2
      DIMENSION EXI(5)
      DATA CON/(0.,.07957747151)/
      CONK=CON*ETA/XK
...
C
      COMPLEX XK,XKS,EXI,XKJ,XKSJ,RI2,RI4,RI5,EXX
...
C
      CALL ROMBG(Z1,Z2,5,GF,EXI,DMIN,1.E-5)
C
...
C
      COMPLEX EX,XKJ,XKSJ,RKJ,ZA
      COMMON /TMI/XKJ,XKSJ,ZZ,RH2,IJ
      DIMENSION EX(5)
      DATA C1,C2,C3,C4,C5,C6,C7,C8/1.,.5,.166666666667,4.1666666667E-2,
     &8.33333333E-3,1.388888889E-3,1.98412698E-4,2.48015873E-5/
      ZDM=Z-ZZ
      R=SQRT(RH2+ZDM*ZDM)
      RKJ=-XKJ*R
      IF (IJ.NE.0)THEN
...
C     (CONSTANT CURRENT PRODUCES ZERO RHO COMPONENT OF E)
C
      COMPLEX XK,ETA,EZK,EZS,EZCM,ERS,ERC,CON,FJ,A1,AR1,A2,CONR,D1,D2,
     &D3,AR2
      DATA CON/(0.,.07957747151)/,FJ/(0.,1.)/
C
...
C     (CONSTANT CURRENT PRODUCES ZERO RHO COMPONENT OF E)
C
      COMPLEX XK,ETA,ERS,ERC,CON,FJ,XKJ,EXKZ,EXMKD,EXPKD,COSKD,
     &SINKD,TERM1,TERM2,TERM3,TERM4,XK2,XK3
      DATA CON/(0.,.079577471)/,FJ/(0.,1.)/
      SH=.5*SEGL
      XKJ=FJ*XK
      ABZ=ABS(ZOB)
      EXKZ=EXP(-XKJ*ABZ)
...
C
      COMPLEX XK,EXRI,XKJ,XJS,RX,TM1,TM2,TM3,TM4,TM5
      XKJ=-(0.,1.)*XK
      RHS=RHO*RHO
      SH=.5*SLEN
...
C
      ...
      RZ=SQRT(RHS+ZOB*ZOB)
      RX=XKJ*RZ
      TM1=(((.04166667*RX-.1666667)*RX+.5)*RX-1.)*RX+1.
      TM2=-XKJ*(((.1666667*RX-.5)*RX+1.)*RX-1.)
      TM3=XJS*((.25*RX-.5)*RX+.5)
      TM4=.1666667*XKJ*XJS*(1.-RX)
      TM5=.04166667*XJS*XJS
...
C     (CONSTANT CURRENT PRODUCES ZERO RHO COMPONENT OF E)
C
      COMPLEX XK,ETA,EZK,EZS,EZCM,ERS,ERC,CON,FJ,CONK,A1,A2,A3,A4,EX1,
     &EX2,ESUM,EDIF,EORDFX,EORSMX,EORDIF,EORSUM,EINT,SHXK,SHXK3,TXK3,
     &COSKD,SINKD,XK2,XK3,TERM1
      DATA CON/(0.,.07957747151)/,FJ/(0.,1.)/
      CONK=CON*ETA
      SH=.5*SLEN
      RHS=RHO*RHO
...
C
      ...
      IF(RHO.GT..04*AMIN1(R1,R2).OR.(S1.LT.0.AND.S2.GT.0.))THEN
...
C
      COMPLEX XK,ETA,XKS,EZK,EZS,EZCM,ERK,ERS,ERCM,CON,FJ,XKR,EPTQ,EPTZ,
     &EPTR,SINKS,COSKSM,CCOSM1
      DATA CON/(0.,.07957747151)/,FJ/(0.,1.)/
      ZDIF=ZOB-ZPTQ
      RSQ=RHO*RHO+ZDIF*ZDIF
      R=SQRT(RSQ)
      XKR=FJ*XK*R
      EPTQ=-CON*ETA/XK*(1.+XKR)*EXP(-XKR)/RSQ*SIGN(1.,ZPTQ)
      EPTZ=EPTQ*ZDIF/R
      EPTR=EPTQ*RHO/R
      SINKS=SIN(XKS*ZPTQ)
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX CURX,AIX,BIX,CIX,CUR,XKS,AX,BX,CX,XKU,XKL,ETAU,ETAL,CEPSU,
     &CEPSL,ETAL2,FRATI,CCJ,CURD,VOLTS,CS1,CS2
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
     ...
     &ICLIFT,CLIFL,CLIFH,EPSR2,SIG2,NRADL,SCNRAD,SCNWRD,KSYMP,IFAR,
     &IPERF,GSCAL
      DIMENSION T1X(1),T1Y(1),T1Z(1),CURX(*)
      EQUIVALENCE (T1X,SI),(T1Y,ALP),(T1Z,BET)
      DATA CCJ/(0.,-0.01666666667)/
      IF (N.EQ.0) GO TO 6
      DO 1 I=1,N
      CURX(I)=CURX(I)/XKU
      AIX(I)=(0.,0.)
...
2     CIX(J)=CIX(J)+CX(JX)*CURD
      IF(NSORC.GT.0)THEN
         ...
         ICON1(I)=JX
         SH=SI(I)*.5
         VOLTS=CMPLX(PSOR2(IS),PSOR3(IS))
         CURD=XKS(I)*SH
         CURD=CCJ*VOLTS/((ALOG(2.*SH/BI(I))-1.)*(BX(JSNO)*COS(CURD)+
     &   CX(JSNO)*SIN(CURD)))
         DO 3 JX=1,JSNO
         J=JCO(JX)
         AIX(J)=AIX(J)+AX(JX)*CURD
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX ETH,EPI,ERD,AIX,BIX,CIX,CUR,XKS,XKU,XKL,ETAU,ETAL,CEPSU,
     &CEPSL,FRATI,ETAL2,XK,ERV,EZV,ERH,EPH,EZH,DIPM,EX,EY,EZ
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2,QSEGE1,
     &QSEGE2,QJRATI,XK
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
...
1     QSEGE2(ISEG)=(0.,0.)
      ...
      DO 2 IEND=1,2
      IF(IEND.EQ.1)THEN
         IF(REAL(QSEGE1(ISEG)).NE.0.)GO TO 2
         IENDX=-1
      ELSE
         IF(REAL(QSEGE2(ISEG)).NE.0.)GO TO 2
         IENDX=1
      END IF
C
...
C
         ...
         IF(JNCEND(J).GT.0)THEN
            IF(ISETJN.EQ.1)THEN
               QSEGE2(JSEG)=REAL(QJRATI(J))
            ELSE
               QSEGE2(JSEG)=1./(CLOG(2./(XK*BI(JSEG)))-.577215664)
            END IF
         ELSE
            IF(ISETJN.EQ.1)THEN
               QSEGE1(JSEG)=REAL(QJRATI(J))
            ELSE
               QSEGE1(JSEG)=1./(CLOG(2./(XK*BI(JSEG)))-.577215664)
            END IF
         END IF
4        CONTINUE
      ELSE
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX AX,BX,CX,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,FRATI,ETAL2,
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX AA,BB,CC,XKU,XKL,ETAU,ETAL,CEPSU,CEPSL,ETAL2,FRATI,
     &AIX,BIX,CIX,CUR,XKS,XK,D,CDH,SDH,CD,SD,OMC,AJ,AP,PP,PM,QP,QM,DEN,
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX XK,PMATX,RHS,SPP,SPM,SEMINF
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
      DIMENSION CAB(1),SAB(1),JNCSEG(*),JNCEND(*),PMATX(JNDIM,JNDIM),
     &RHS(JNDIM)
      EQUIVALENCE (CAB,ALP),(SAB,BET)
      NTOT=NJWIRE*NSEGPW
      DO 2 IW=1,NTOT
...
C     SPM = SCALAR POTENTIAL DUE TO CHARGE DISTRIBUTION OF .5*SLEN-Z
C
      COMPLEX XK,SPP,SPM,XKJ,XJS,RX,TM1,TM2,TM3,TM4,TM5,EXPR,EXR0,EXR1
      XKJ=-(0.,1.)*XK
      RHS=RHO*RHO
      SH=.5*SLEN
      S1=-SH-ZOB
...
C
      SAVE A1,A3,M
      COMPLEX CLOGZ,H0,J0,Y0,Z,ZI,ZK
      DIMENSION M(101), A1(25), A3(25)
      DATA PI,GAMMA,C2/3.141592654,.5772156649,.3674669052/,INIT/0/
...
C
      SAVE A1,A2,A3,A4,M
      COMPLEX CLOGZ,H0,H0P,J0,J0P,P0Z,P1Z,Q0Z,Q1Z,Y0,Y0P,Z,ZI,ZI2,ZK,FJX
      DIMENSION M(101), A1(25), A2(25), A3(25), A4(25)
      DATA PI,GAMMA,C1,C2,C3,P10,P20/3.141592654,.5772156649,
     &-.0245785095,.3674669052,.7978845608,.0703125,.1121520996/
      DATA Q10,Q20,P11,P21,Q11,Q21/.125,.0732421875,.1171875,.1441955566
     &,.375,.1025390625/
      DATA POF,INIT/.7853981635,0/
      IF (INIT.EQ.0) GO TO 5
1     FJX=(0.,1.)
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX CM,SSX,D,DETER,AX,BX,CX
      COMMON /DATA/ LD,N1,N2,N,NP,M1,M2,M,MP,X(MAXSEG),Y(MAXSEG),
     &Z(MAXSEG),SI(MAXSEG),BI(MAXSEG),ALP(MAXSEG),BET(MAXSEG),
     &SALP(MAXSEG),T2X(MAXSEG),T2Y(MAXSEG),T2Z(MAXSEG),ICON1(MAXSEG),
     &ICON2(MAXSEG),ITAG(MAXSEG),ICONX(MAXSEG),IPSYM
...
C
      COMPLEX FJ,EKR1,EKR2,T1,T2,CONS,HPS,HPC,HPK,XK,XKJ,CDK,SDK,DK
      COMMON /TMH/ XKJ,ZP,RH2
      EXTERNAL GH
      DATA FJ/(0.,1.)/,PI4/12.56637062/,PI8/25.13274123/
      ...
      CDK=COS(DK)
      SDK=SIN(DK)
      CALL ROMBG (-DH,DH,1,GH,HPK,0.,1.E-4)
      HPK=HPK*RH/PI4
...
C
      INCLUDE 'NECPAR.INC'
      COMPLEX CM,H1X,H1Y,H1Z,H2X,H2Y,H2Z,EXC,EYC,EZC,EMEL,CW,
     ...
     &T1YJ,T1ZJ,T2XJ,T2YJ,T2ZJ,IPGND
      COMMON /SEGJ/ AX(NSJMAX),BX(NSJMAX),CX(NSJMAX),JCO(NSJMAX),JSNO,
     &ISCON(NSCNGF),NSCON,IPCON(NSPNGF),NPCON
      DIMENSION CAB(1), SAB(1), CM(NROW,*), CW(NROW,*)
      DIMENSION T1X(1),T1Y(1),T1Z(1),EMEL(9)
...
C
      SAVE
      COMPLEX CKSM,CCK1,CCK1S,CCK2,CCK2S,CON1,CON2,CON3,XLA,GCK1,GCK1SQ,
     &GCK2,GCK2SQ,EPSC1
...
C
      ...
      DATA C3,P10,P20,Q10,Q20/.7978845608,.0703125,.1121520996,.125,
     &.0732421875/
      DATA P11,P21,Q11,Q21/.1171875,.1441955566,.375,.1025390625/
      DATA POF,INIT/.7853981635,0/,FJ/(0.,1.)/
      IF (INIT.EQ.0) GO TO 5
1     ZMS=Z*CONJG(Z)
...
C     FORTRAN ABS,AIMAG,AMAX1,ABS,CMPLX,CSQRT,MIN0,REAL
C
C     INTERNAL VARIABLES
C
...
C
      ...
      MAXJ=L
      DO 10 J=L,PU
      IF (REAL(QRAUX(J)).LE.MAXNRM) GO TO 9
      MAXNRM=REAL(QRAUX(J))
      MAXJ=J
9     CONTINUE
10    CONTINUE
...
C
      SAVE
      COMPLEX ERV,EZV,ERH,EPH,EZH,A,B,CK1,CK1SQ,CK2,CK2SQ,BK,SUM,CP1,CP2
     &,CP3,CKSM,CON1,CON2,CON3,XJK,EXJK,A1,A2,A3,A4,A5,B1,B2,B3,B4,B5,C1
     &,C2,C3,C4,C5,RF1,RF2,SDIR,CKDSQ,D12,FJ
     ...
     &G,CKAR,CKAI,CKBR,CKBI,ZZ,ZP,RHO,JH,IQAZ
      ...
      DIMENSION SUM(6)
      DATA CK1R,CK1I,CK2R,CK2I,PTP,ACC/0.,0.,0.,0.,1.5707,1.E-5/
      IF (REAL(CK1).NE.CK1R.OR.AIMAG(CK1).NE.CK1I) GO TO 1
      IF (REAL(CK2).NE.CK2R.OR.AIMAG(CK2).NE.CK2I) GO TO 1
      GO TO 4
...
10    JH=1
      ...
      CP1=CMPLX(CKAR-RMIS,CKAI-RMIS)
      CP2=CMPLX(DMIS,CKAI-RMIS)
      XLI=ALOGA/RHO
      SDIR=CMPLX(ZMP,RHO)/SQRT(RHO*RHO+ZMP*ZMP)
      DEL=PTP/DEL
      IF (DEL.GT.CKAR) DEL=CKAR
      CKDSQ=SQRT(CK1SQ-CK2SQ)
      D12=CKDSQ*CMPLX(ZZ,-ZP)-FJ*(CK1-CK2)*RHO
      XLN=XLI-REAL(D12)/RHO
      AAD12=ABS(AIMAG(D12))
      IF (AAD12.LT.12.57) GO TO 14
      IF (RHO.LT.1.E-5) GO TO 14
      IF (ZMP.LT.1.E-10) GO TO 11
      IF (RHO*AAD12.LT.4.*ZZ*REAL(D12)) GO TO 14
      IF (XLN.LE.0.) GO TO 11
      A=-SQRT((0.,2.)*CK1)*ZP
      B=FJ*CK1*ZZ/CKDSQ+RHO
      BK=B*XLN-A*SQRT(XLN)
      RMIS=4.*ABS(AIMAG(BK))*REAL(D12)
      IF (REAL(BK)*AAD12.LT.RMIS) GO TO 14
      RMIS=REAL(A)
      IF (RMIS*RMIS.GT.4.*REAL(B)) GO TO 14
...
12    SUM(I)=-SUM(I)
      B=CP1
      CALL ROM1 (6,SUM,1.,0,ICYES)
C     PATH TO -INFINITY
      SDIR=-CONJG(SDIR)
      A=CP1
      B=A+SDIR*(XLI-AIMAG(A))/AIMAG(SDIR)
      DZS=DEL/ABS(B-A)
      CALL ROM1 (6,SUM,DZS,1,ICYES)
      DO 13 I=1,6
13    SUM(I)=-SUM(I)
...
14    A=CP2
      B=CP3
      IF(ABS(AIMAG(CP3)).GT.XLI)B=CP2+XLI*(CP3-CP2)/AIMAG(CP3-CP2)
      DZS=DEL/ABS(B-A)
      CALL ROM1 (6,SUM,DZS,1,ICYES)
      IF (ICYES.EQ.1) GO TO 15
      A=CP3
      B=A+SDIR*(XLN-AIMAG(A)+CKBI)/AIMAG(SDIR)
      DZS=DEL/ABS(B-A)
      CALL ROM1 (6,SUM,DZS,1,ICYES)
15    DO 16 I=1,6
16    SUM(I)=-SUM(I)
...
C     EXP(+JWT).
19    ERV=CONJG(CON1*SUM(1))*R
      EZV=CONJG(CON1*SUM(2))*R
      ERH=CONJG(CON1*(SUM(3)+SUM(6)))*R
      EPH=-CONJG(CON1*(SUM(4)+SUM(6)))*R
      EZH=-CONJG(CON1*SUM(5))*R
      IF (IQAX.EQ.0) RETURN
...
21    SF3=STH-SF2
      ...
      ERV=ERV-CONJG(EXJK*(CON3*SF1-CON2*SFAC*CTH))
      EZV=EZV-CONJG(EXJK*(CON3-CON2*SFAC*STH))
      ERH=ERH-CONJG(EXJK*(CON3*(SF2-1.)+CON2*SFAC*SF3+1.))
      EPH=EPH-CONJG(-EXJK*((-CON3+CON2*SFAC)*SF2+1.))
      EZH=EZH-CONJG(-EXJK*(CON3*CK1SQ/CK2SQ*SF1+CON2*SFAC*CTH))
      RETURN
22    ERV=0.
      EZV=0.
      ERH=0.
...
C     SQUARES SOLUTION.
      ...
      YMAX=0.
      DO 2 I=1,IEQ
      RMS=REAL(RA(I)*CONJG(RA(I)))
      RMR=RMR+RMS
      YMS=REAL(Y(I)*CONJG(Y(I)))
...
C
     ...
     &), T20(6)
      DATA DZMN,DZMX,RX,RXL,RC/1.E-6,.9999,.5E-3,.5E-4,1.E-4/
      LSTEP=0
      ICYES=0
      Z=0.
      ZE=1.
...
C     COMMON/PTFIT/.
C
      COMPLEX CLS1,CLS2,CLS3,RE1,RE2,RE3,RE4,RE5,FAC1,FAC2,FAC3,FAC4,FAC
     &5,ERV,EZV,ERH,EPH,EZH,EXX,CFF1,CFF2,CFF3,CFF4,CFF5,CFF6,CFF7,CFF8,
     &CFF,XLA,GCK1,GCK1SQ,GCK2,GCK2SQ,CQRA,RA,DUM
     ...
     &FF17,FF18,FF19,FF20,FF21,FF22,FF23,FF24,NFUNF,NFUNC
      COMMON /COFIT/ FAC1(32,3),FAC2(32,3),FAC3(32,3),FAC4(32,3),
     &FAC5(32,3),RKFAC,MFUNF(3),MFUNC(3)
      COMMON /PTFIT/ RH1,DRH,NRH,ZZ1,DZZ,NZZ,ZP1,DZP,NZP,SRH1,DSRH,NSRH,
     &SZZ1,DSZZ,NSZZ,SZP1,DSZP,NSZP,IQAX,RES1,RES2,RES3,RES4,RES5,REX1,R
     &EX2,REX3,REX4,REX5
      COMMON /GPARM/ XLA(21,21),GCK1,GCK1SQ,GCK2,GCK2SQ,ZRAT,RHON
...
C
      COMPLEX ANS,XL,DXL,CGAM1,CGAM2,B0,B0P,COM,CK1,CK1SQ,CK2,CK2SQ,CKSM
     &,DEN1,DEN2,DEN3,A1,A2,A3,A4,A5,B1,B2,B3,B4,B5,C1,C2,C3,C4,C5
      COMPLEX TRM1,TRM2,TRM3,RXL,CON1,CON2,CON3,CEXXP,CEXXM,CEXGD,CEXZD,
     &ZPGD,ZZEXP
      COMMON /EVLCOM/ CKSM,CK1,CK1SQ,CK2,CK2SQ,CON1,CON2,CON3,TKMAG,TSMA
     &G,CKAR,CKAI,CKBR,CKBI,ZZ,ZP,RHO,JH,IQAZ
      COMMON /SRGAM/ A1,A2,A3,A4,A5,B1,B2,B3,B4,B5,C1,C2,C3,C4,C5
...
C
9     IF (AIMAG(COM).GE.0.) CGAM1=-CGAM1
      CEXXP=ZZEXP(CGAM1*ZP-CGAM2*ZZ)
...
C=======================================================================
     ...
     &RES3,RES4,RES5,REX1,REX2,REX3,REX4,REX5
      ...
      DATA RH1/0.,0.,0./,ZZ1/0.,0.,0./,ZP1/0.,0.,0./
      DATA RHOA/0.,.2,.6/,RHOB/.6,.6,2./,ZZA/.2,0.,0./,ZZB/2.,.2,2./
      DATA ZPA/0.,0.,0./,ZPB/.25,.25,.25/,EPSCX/(0.,0.)/,NSFILE/0/
      END
...
C
      SAVE
      COMPLEX FAC1,FAC2,FAC3,FAC4,FAC5,EPSC,XLA,GCK1,GCK1SQ,GCK2,GCK2SQ
      COMMON /COFIT/ FAC1(32,3),FAC2(32,3),FAC3(32,3),FAC4(32,3),
     &FAC5(32,3),RKFAC,MFUNF(3),MFUNC(3)
      COMMON /PTFIT/ RH1,DRH,NRH,ZZ1,DZZ,NZZ,ZP1,DZP,NZP,SRH1,DSRH,NSRH,
     &SZZ1,DSZZ,NSZZ,SZP1,DSZP,NSZP,IQAX,RES1,RES2,RES3,RES4,RES5,REX1,R
     ...
     &DSZZA(3),NSZZA(3),SZP1A(3),DSZPA(3),NSZPA(3)
      DATA RH1A/0.,.2,.6/,DRHA/.2,.1,.2/,NRHA/4,5,5/,NREG/3/
      DATA ZZ1A/.2,0.,0./,DZZA/.2,.1,.2/,NZZA/4,4,4/
      DATA ZP1A/0.,0.,0./,DZPA/.1,.1,.1/,NZPA/2,2,2/
      DATA SRH1A/0.,.2,.6/,NSRHA/5,6,8/,SZZ1A/.2,0.,0./,NSZZA/2,2,2/
      DATA SZP1A/0.,0.,0./,NSZPA/2,3,3/
      IQAX=0
      ZZ1A(1)=ELGND
      DZPA(1)=.4*ELGND
      RH1A(2)=ELGND
...
C     LOOP OVER REGIONS
      ...
      DSZP=DSZPA(IR)
      NSZP=NSZPA(IR)
      IF (EXP(AIMAG(GCK1)*RH1).LT.1.E-8) IFUNC=0
      CALL SECOND (TIM1)
...
C
      SAVE
      COMPLEX GCK1,GCK1SQ,GCK2,GCK2SQ,A1,A2,A3,A4,A5,ERV,EZV,ERH,EPH,
     &EZH,EXX,EPSC,XLA
      COMMON /GRIDY/ A1(1200),A2(1200),A3(1200),A4(1200),A5(1200),RH1(3)
     &,ZZ1(3),ZP1(3),DRH(3),DZZ(3),DZP(3),NRH(3),NZZ(3),NZP(3),NPT(3),DR
     &ZZP(3),INDI(3),ZPM1,ZPM2,ZPMM1,ZPMM2,ZPD1,ZPD2
...
C
      COMPLEX F1,F2,SUM,SX
      IF(ABS(F2).GT.0.)THEN
         IF(ABS((F2-F1)/F2).LT.1.E-7)GO TO 1
      END IF
      SX=SUM+F2
      DEN=ABS(REAL(SX))+ABS(AIMAG(SX))
