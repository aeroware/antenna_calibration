...
#define X_SIZE_CM       35.6    /* x size of the device (in cm.) */
#define Y_SIZE_CM       27.8125 /* y size of the device (in cm.) */
#define X_RES         1152.0    /* x resolution of the device */
#define Y_RES          900.0    /* y resolution of the device */
#define BACKGRD_COLOR 0		/* background color */
#define FOREGRD_COLOR 1		/* foreground color */


/* Set up device characteristic data array */
static float characteristics[] = {  3.160,	/* device id */
...
static short cursor_data[16] = {
     ...
     mpr_static(sun2_cursor_image, 16, 16, 1, cursor_data);

     static struct cursor cross_hair_cursor = 
     {
        8, 8, PIX_SRC^PIX_DST, &sun2_cursor_image
     };
...
extern int sun2_init_device();	/* initialize device */
extern int sun2_clear_page();		/* screen clear */
extern int sun2_move_to();		/* absolute window coordinates */
extern int sun2_draw_to();		/* absolute window coordinates */
extern int sun2_flush();		/* flush all pending buffers */
extern int sun2_release_device();	/* close the sun window */
extern int sun2_return_device();	/* return device characteristics */
extern int sun2_select_color();	/* set the current forground color */
extern int sun2_get_input();		/* get input, null routine */
extern int sun2_set_color_map_rgb();	/* set up a rgb color look up table */
extern int sun2_set_color_map_hls();	/* set up a hls color look up table */
extern int sun2_button_input();	/* a routine to handle button input */
extern int sun2_draw_polygon();	/* draw a closed polygon */



gdsun2_( op_code, x_data, y_data )
                      ...
                      (* y_data)[]; /* y coordinate data */
{
   ...
   static int (* jump_table[MAX_OPCODE])() = {
                 ...
                 sun2_select_color,
                 sun2_get_input,
                 sun2_set_color_map_rgb,
                 sun2_set_color_map_hls,
                 sun2_button_input,
                 sun2_draw_polygon };
...
sun2_init_device( op_code, x_data, y_data )
...
float (* x_data)[], (* y_data)[];
{
   if ( gfx == NULL )		/* if first time thru the init device */
   {
      ...
      {
         signal( SIGWINCH, sun2_catch_signal );
...
sun2_clear_page( op_code,  x_data, y_data )
...
sun2_move_to( op_code, x_data, y_data )
...
sun2_draw_to( op_code, x_data, y_data )
...
sun2_flush( op_code, x_data, y_data )
...
sun2_release_device( op_code, x_data, y_data )
...
sun2_return_device( op_code, x_data, y_data )
...
sun2_select_color( op_code, x_data, y_data )
...
sun2_check_input(  gfx,  ibits,  obits,  ebits,   timer )
...
sun2_get_input( op_code, x_data, y_data )
...
sun2_set_color_map_rgb( op_code, x_data, y_data )
...
sun2_set_color_map_hls( op_code, x_data, y_data )
...
sun2_button_input( op_code, x_data, y_data )
...
sun2_draw_polygon( op_code, x_data, y_data )
