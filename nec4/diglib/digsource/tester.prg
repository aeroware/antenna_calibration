...
COMMENT---- draw a circle
      ...
      CALL GSPSTR('Test: Draw a Circle')
      XSTART=.5*GSXLCM(0)
      YSTART=.25*GSYLCM(0)
...
COMMENT---- draw the character set
      ...
      CALL GSPSTR('abcdefghij')
      CALL GSMOVE(0.5,8.0)
      CALL GSPSTR('klmnopqrst')
      CALL GSMOVE(0.5,6.0)
      CALL GSPSTR('uvwxyz{|}')
C
      CALL GSSETC(GOODCS(0.1),0.0)
...
C
      ...
      CALL MAPIT(0.0,10.0,0.0,100.0,'X AXIS','Y AXIS','PLOT 1',0)
      CALL SAVMAP(AREA(1))
C
      CALL MAPSIZ(0.0,100.0,52.5,100.0,goodcs(0.))
      CALL MAPIT(0.0,10.0,0.0,1000.0,'X AXIS','Y AXIS','PLOT 2',0)
      CALL SAVMAP(AREA(16))
C
      DO 100 I=1,101
            X = (I-1)/10.0
...
C
  ...
  103 CONTINUE
      ...
      CALL MAPIT(XMIN,XMAX,YMIN1,YMAX1,'X AXIS','LEFT Y AXIS',
     1   'PLOT TITLE',0)
      CALL CURVEY(XMIN,XMAX,Y,101,3,0.3,10)
      CALL SYAXIS(YMIN2,YMAX2,'RIGHT Y AXIS',IAXES)
...
C
      ...
      CALL MAPIT(-1.,2.0,-1.0,2.0,'X','Y',
     1   'VERTICES AT (0,0) and (1,1) -- space=again non-space=quit',0)
      CALL CLLINE(0.,0., 1.,0.)
      CALL CLLINE(1.,0., 1.,1.)
      CALL CLLINE(1.,1., 0.,1.)
      CALL CLLINE(0.,1., 0.,0.)
...
C
      ...
      CALL MAPIT(1.0,10.0,1.0,100.0,'X AXIS','Y AXIS','PLOT 1',0)
      CALL GSCOLR(2,IERR)
...
C
      ...
      CALL MAPIT(1.0,10.0,1.0,1000.0,'X AXIS','Y AXIS','PLOT 2',0)
      CALL GSCOLR(3,IERR)
      CALL CURVEY(1.0,10.0,Y2,101,2,0.3,5)
      CALL GSCOLR(1,IERR)
      CALL MAPSIZ(0.0,100.0,0.0,47.5,goodcs(0.))
      CALL MAPIT(1.0,10.0,1.0,1000.0,'X AXIS','Y AXIS','PLOT 3',2)
      CALL GSCOLR(4,IERR)
      CALL CURVEY(1.0,10.0,Y1,101,1,0.3,10)
      CALL GSCOLR(5,IERR)
      CALL CURVEY(1.0,10.0,Y2,101,2,0.3,5)
