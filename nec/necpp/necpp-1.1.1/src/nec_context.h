...
public:
	...
	*/
	...
	{
		geometry.arc(tag_id, segment_count, rada, ang1, ang2, rad);
	}
	...
	*/
	...
	{
		geometry.helix(s, hl, a1, b1, a2, b2, rad, segment_count, tag_id);
	}
...
//	int ifrtmp;
	...
	/* common  /fpat/ */
	int m_near, nfeh, nrx, nry, nrz, nth, nph, ipd, iavp, inor, iax, excitation_type;
	nec_float thets, phis, dth, dph, rfld, gnor; 
	nec_float xpr6, structure_power_loss, xnr, ynr, znr, dxnr, dynr, dznr;
	
	
	/* common  /dataj/ */
	int ind1, indd1, ind2, indd2, ipgnd;
...
private:
	...
	void cmset(int nrow, complex_array& cm, nec_float rkhx);
	...
	void netwk(complex_array& cm, int_array& ip, complex_array& einc);
	void nfpat(void);
