	...
	*/
	...
	{	
		...
		case 1: /* Memory allocation for primary interacton matrix. */
			...
			geometry.frequency_scale(freq_mhz);
			processing_state = 2;
		...
		for (int i = 0; i < geometry.n; i++ )
		{
			...
			else
			{
				...
				m_output.nec_printf(
					...
					i+1, geometry.segment_tags[i],
					geometry.x[i], geometry.y[i], geometry.z[i], geometry.segment_length[i],
	...
	*/
	...
	while( true )
	{
		...
		for(int i = l1-1; i < l2; i++ )
		{
			...
			ASSERT(0.0 != geometry.segment_length[i]);
			ASSERT(0.0 != geometry.segment_radius[i]);
			...
			switch( jump )
			{
				...
				case 6:
				{
					zt= zint( zlr[istepx]* wavelength, geometry.segment_radius[i]);
				}
    ...
    int nr, complex_array& cw, int nw, int itrp )
{
	...
	j--;
	m_s= geometry.segment_length[j];
	m_b= geometry.segment_radius[j];
	xj= geometry.x[j];
	yj= geometry.y[j];
	zj= geometry.z[j];
	cabj= geometry.cab[j];
	...
	for(int i = i1; i <= i2; i++ )
	{
		...
		if ( itrp == 0)
		{
			for(int ij = 0; ij < geometry.jsno; ij++ )
			{
    ...
    int nr, complex_array& cw, int nw, int itrp)
{
	...
	if ( m_use_exk == true)
	{
		...
		if ( ipr < 0 )
		{
			...
			{
				ind1 = geometry.test_ek_approximation(j,iprx);
#ifdef NEC_ERROR_CHECK
				int ind_test;
				xi= fabs( cabj* geometry.cab[iprx]+ sabj* geometry.sab[iprx]+ salpj* geometry.salp[iprx]);
				if ( (xi < 0.999999) || (fabs(geometry.segment_radius[iprx]/m_b-1.0) > 1.e-6) )
					ind_test=2;
				else
					ind_test=0;
		...
		if ( ipr < 0 )
		{
			...
			{
				ind2 = geometry.test_ek_approximation(j,iprx);
#ifdef NEC_ERROR_CHECK
				int ind_test;
				xi= fabs( cabj* geometry.cab[iprx]+ sabj* geometry.sab[iprx]+ salpj* geometry.salp[iprx]);
				if ( (xi < 0.999999) || (fabs(geometry.segment_radius[iprx]/m_b-1.0) > 1.e-6) )
...
/* constant currents.  The ground effect is included. */
...
void nec_context::hintg( nec_float xi, nec_float yi, nec_float zi )
{
	...
	for( ip = 1; ip <= ground.ksymp; ip++ )
	{
		...
		cr= cos( rk);
		sr= sin( rk);
		gam=-( nec_complex(cr,-sr)+rk*nec_complex(sr,cr) )/( FPI*rsq*r )* m_s;
		exc= gam* rx;
		eyc= gam* ry;
		ezc= gam* rz;
		t1zr= t1zj* rfl;
    ...
    nec_complex *ex, nec_complex *ey, nec_complex *ez )
{
	...
	*ez=cplx_00();
	...
	{
		ix = i+1;
		m_s = geometry.segment_length[i];
		m_b = geometry.segment_radius[i];
		xj= geometry.x[i];
		yj= geometry.y[i];
		zj= geometry.z[i];
		cabj= geometry.cab[i];
		...
		if ( ipr < 0 )
		{
			...
			{
				ind1 = geometry.test_ek_approximation(i,iprx);
#ifdef NEC_ERROR_CHECK
				int ind_test;
				xi= fabs( cabj* geometry.cab[iprx]+ sabj* geometry.sab[iprx]+ salpj* geometry.salp[iprx]);
				if ( (xi < 0.999999) || (fabs(geometry.segment_radius[iprx]/m_b-1.) > 1.0e-6) )
					ind_test=2;
				else
					ind_test=0;
				ASSERT(ind_test == ind1);
		...
		else
		{
			...
			if ( ipr != ix )
			{
				...
				{
					ind2 = geometry.test_ek_approximation(i,iprx);
#ifdef NEC_ERROR_CHECK
					int ind_test;
					xi= fabs( cabj* geometry.cab[iprx]+ sabj* geometry.sab[iprx]+ salpj* geometry.salp[iprx]);
					if ( (xi < 0.999999) || (fabs(geometry.segment_radius[iprx]/m_b-1.) > 1.0e-6) )
...
/* present. */
void nec_context::netwk( complex_array& cm, int_array& ip,
    complex_array& einc )
	/*
...
*/
		...
		nec_float segment_length_phase = geometry.segment_length[segment_index] * pi(); 
...
void nec_context::qdsrc( int is, nec_complex v, complex_array& e )
{
	...
	geometry.tbf( is+1,0);
	geometry.icon1[is]= i;
	m_s = geometry.segment_length[is]*.5;
	curd= s_CCJ * v/(( log(2.* m_s/ geometry.segment_radius[is])-1.)*( geometry.bx[geometry.jsno-1]*
		cos( two_pi() * m_s)+ geometry.cx[geometry.jsno-1]* sin( two_pi() * m_s))* wavelength);
	vqds[nqds]= v;
	iqds[nqds]= is+1;
	nqds++;
	...
	for( jx = 0; jx < geometry.jsno; jx++ )
	{
		...
		if ( ipr < 0 )
		{
			...
			{
				ind1 = geometry.test_ek_approximation(j,ipr);
				
				int ind_test;
				xi= fabs( cabj* geometry.cab[ipr]+ sabj* geometry.sab[ipr]+ salpj* geometry.salp[ipr]);
				if ( (xi < 0.999999) || (fabs(geometry.segment_radius[ipr]/m_b-1.) > 1.0e-6) )
					ind_test=2;
				else
					ind_test=0;
				ASSERT(ind_test == ind1);
		...
		else /* ipr > 0 */
		{
			...
			if ( ipr != j )
			{
				...
				{
					ind2 = geometry.test_ek_approximation(j,ipr);
...
void nec_context::print_radiation_pattern(nec_float pinr, nec_float pnlr)
{
	...
	for( kph = 1; kph <= nph; kph++ )
	{
		...
		for( kth = 1; kth <= nth; kth++ )
		{
			...
			ethm2= norm(eth); // real( eth* conj( eth));
			ethm= sqrt( ethm2);
			etha= arg_degrees( eth);
			ephm2= norm(eph); // real( eph* conj( eph));
			ephm= sqrt( ephm2);
			epha= arg_degrees( eph);
