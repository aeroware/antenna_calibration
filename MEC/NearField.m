function [E_r,E_t,B]=NearField(ant,f, I, NN,dist,output)

%   function [E_r,E_t,B]=FarField(ant,f, I, NN,dist,output)
%
...
%   compute fields
        ...
        for(n=1:N)
            ...
            % compute fields
                    E_r(t,p)=E_r(t,p)+((Z0*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*((1/(i*k*s))+(1/(i*k*s)^2))*2*costheta;
                    E_t(t,p)=E_t(t,p)+((Z0*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*(1+(1/(i*k*s))+(1/(i*k*s)^2))*sin(realtheta);
                    B(t,p)=B(t,p)+((mu*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*(1+(1/(i*k*s)))*sin(realtheta);
                end
            end
...
% plot graph
        ...
        polar(theta(1,:),abs(real(E_r(:,1))'));
        hold on
        polar(-theta(1,:),abs(real(E_r(:,1+floor(NN/2))')));
        hold off

        title('Radial part of Electric field as function of colatitude');

        figure
        for t=1:NN
            for p=1:NN
                x(t,p)=real(E_r(t,p))*cos(phi(p))*sin(theta(t));
                y(t,p)=real(E_r(t,p))*sin(phi(p))*sin(theta(t));
                z(t,p)=real(E_r(t,p))*cos(theta(t));
            end
        end
        ...
        surf(x,y,z,real(E_r))
        colorbar
        title('Radial part of Electric field, color coded');
        
        figure
        for t=1:NN
            for p=1:NN
                x(t,p)=abs(real(E_r(t,p)))*cos(phi(p))*sin(theta(t));
                y(t,p)=abs(real(E_r(t,p)))*sin(phi(p))*sin(theta(t));
                z(t,p)=abs(real(E_r(t,p)))*cos(theta(t));
            end
        end
        surf(x,y,z,abs(real(E_r)))
        colorbar
        title('Magnitude of Radial part of Electric field, color coded');

        figure
        polar(theta(1,:),abs(real(E_t(:,1))'));
        hold on
        polar(-theta(1,:),abs(real(E_t(:,1+floor(NN/2))')));
        hold off

        title('Theta component of Electric field as function of colatitude');

        figure
        for t=1:NN
            for p=1:NN
                x(t,p)=real(E_t(t,p))*cos(phi(p))*sin(theta(t));
                y(t,p)=real(E_t(t,p))*sin(phi(p))*sin(theta(t));
                z(t,p)=real(E_t(t,p))*cos(theta(t));
            end
        end
        surf(x,y,z,real(E_t))
        colorbar
        title('Theta component of Electric field, color coded');
        
        figure
        for t=1:NN
            for p=1:NN
                x(t,p)=abs(real(E_t(t,p)))*cos(phi(p))*sin(theta(t));
                y(t,p)=abs(real(E_t(t,p)))*sin(phi(p))*sin(theta(t));
                z(t,p)=abs(real(E_t(t,p)))*cos(theta(t));
            end
        end
        surf(x,y,z,abs(real(E_t)))
        colorbar
        title('Magnitude of Theta component of Electric field, color coded');
        ...
        for t=1:NN
            for p=1:NN
                x(t,p)=real(B(t,p))*cos(phi(p))*sin(theta(t));
                y(t,p)=real(B(t,p))*sin(phi(p))*sin(theta(t));
                z(t,p)=real(B(t,p))*cos(theta(t));
            end
        end
        surf(x,y,z,real(B))
        colorbar
        title('Magnetic induction field, color coded');
        ...
        surf(x,y,z,abs(real(B)))
        colorbar
        title('Magnitude of Magnetic induction field, color coded');
    case(2)
        E_r=zeros(NN,NN,NN);
        E_t=zeros(NN,NN,NN);
        B=zeros(NN,NN,NN);
            ...
            for(t=1:NN)
                for(p=1:NN)
                    ...
                    % compute distance s and the real angle
                        ...
                        costheta=(point(3)-mid(n,3))/s;
            % compute fields
                        E_r(t,p,rr)=E_r(t,p,rr)+((Z0*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*((1/(i*k*s))+(1/(i*k*s)^2))*2*costheta;
                        E_t(t,p,rr)=E_t(t,p,rr)+((Z0*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*(1+(1/(i*k*s))+(1/(i*k*s)^2))*sin(realtheta);
                        B(t,p,rr)=B(t,p,rr)+((mu*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*(1+(1/(i*k*s)))*sin(realtheta);
                    end
                end
            end
        end % for all segments
        ...
        for(n=1:N)
            ...
            for(t=1:NN)    
                for(rr=1:NN)
                    ...
                    costheta=(point(3)-mid(n,3))/s;
            % compute fields
                    E_r_v(t,rr)=E_r_v(t,rr)+((Z0*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*((1/(i*k*s))+(1/(i*k*s)^2))*2*costheta;
                    E_t_v(t,rr)=E_t_v(t,rr)+((Z0*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*(1+(1/(i*k*s))+(1/(i*k*s)^2))*sin(realtheta);
                    B_v(t,rr)=B_v(t,rr)+((mu*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*(1+(1/(i*k*s)))*sin(realtheta);
                end
            end
            ...
            for(p=1:NN)    
                for(rr=1:NN)
                    ...
                    costheta=(point(3)-mid(n,3))/s;
            % compute fields
                    E_r_h(p,rr)=E_r_h(p,rr)+((Z0*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*((1/(i*k*s))+(1/(i*k*s)^2))*2*costheta;
                    E_t_h(p,rr)=E_t_h(p,rr)+((Z0*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*(1+(1/(i*k*s))+(1/(i*k*s)^2))*sin(realtheta);
                    B_h(p,rr)=B_h(p,rr)+((mu*k*I(n)*l(n)*i)/(4*pi*s))*exp(-i*k*s)*(1+(1/(i*k*s)))*sin(realtheta);
                end
            end
